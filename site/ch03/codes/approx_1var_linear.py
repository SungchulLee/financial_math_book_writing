"""
Approx 1Var Linear

Educational script demonstrating approx 1var linear concepts.
"""

# ============================================================================
# APPROXIMATE_ONE_VARIABLE_FUNCTION_LINEARLY.py
# ============================================================================
import matplotlib.pyplot as plt
import numpy as np

# Define x range


if __name__ == "__main__":
    x = np.linspace(0., 2.)

    # Define functions
    f = np.exp(x - 1) + (x - 1)**2             # Original nonlinear function
    g = x                                      # Linear approximation (tangent at x=1)

    fig, ax = plt.subplots(figsize=(12, 6))
    ax.plot(1, 1, 'or', label='Tylor Expansion Point (x=1)')
    ax.plot(x, f, label=r'Original Function: $f(x) = e^{x-1} + (x-1)^2$')
    ax.plot(x, g, 'r--', label=r'Linear Approximation: $g(x) = x$')
    ax.set_title(r"Original Function vs Linear Approximation at $x=1$", fontsize=14)
    ax.set_xlabel("x", fontsize=14)
    ax.set_ylabel("y", fontsize=14)
    ax.grid(True, alpha=0.3)
    ax.legend(loc=(0.1,0.7), fontsize=14)
    for spine in ["top","right"]:
        ax.spines[spine].set_visible(False)
    for spine in ["bottom","left"]:
        ax.spines[spine].set_position("zero")
    plt.tight_layout()
    plt.show()
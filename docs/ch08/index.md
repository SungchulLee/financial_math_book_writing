...
### **Stability** requires bounded error growth: for all $n\Delta t \leq T$ 

$$\|B^n\| \leq C$$ 

### **Von Neumann stability analysis** inserts a Fourier mode $V_j^n = g^n e^{ikj\Delta S}$ and requires the amplification factor $|g(\xi)| \leq 1$ for all frequencies. For the explicit scheme on the heat equation, $g = 1 - 2\lambda(1-\cos k\Delta x)$ with $\lambda = \Delta t/(\Delta x)^2$, yielding the **CFL condition** $\lambda \leq 1/2$. For the implicit scheme, $g = 1/(1 + 2\lambda(1-\cos k\Delta x)) < 1$ unconditionally. For Crank-Nicolson, $g = (1-\lambda(1-\cos k\Delta x))/(1+\lambda(1-\cos k\Delta x))$, also unconditionally stable. The CFL condition for Black-Scholes with variable coefficients is $\sigma^2 S_{\max}^2 \Delta t/(2(\Delta S)^2) \leq 1/2$. **Monotone schemes** preserve the discrete maximum principle, implying $L^\infty$ stability. Non-smooth payoffs reduce observed convergence order (from 2 to 0.5--1 for Crank-Nicolson near kinks), motivating Rannacher smoothing, payoff smoothing, grid adaptation, and Richardson extrapolation. **Grid convergence** is verified by refining $\Delta S$ and $\Delta t$ and plotting error on log-log scales; a ratio of $\approx 4$ in successive errors confirms second-order convergence. **Richardson extrapolation** combines solutions at two grid spacings to cancel leading error terms, boosting accuracy by one order: $V_{\text{ext}} = 2V(2M) - V(M)$.

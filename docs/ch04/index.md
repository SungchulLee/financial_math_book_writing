...
This chapter develops the mathematical and financial framework for **changing probability measures in continuous time**, culminating in the construction of the **risk-neutral measure** used for derivative pricing.

We begin by building the **martingale machinery**: local martingales, stochastic exponentials, and the integrability conditions (Novikov and Kazamaki) that ensure valid measure changes. These tools establish the technical foundation for transforming probability measures.

We then connect martingale theory to finance through the **Fundamental Theorem of Asset Pricing**, showing that the absence of arbitrage is equivalent to the existence of a probability measure under which discounted asset prices are martingales. This leads directly to the **risk-neutral valuation principle**, which expresses derivative prices as conditional expectations under an equivalent measure.

With this foundation in place, we introduce **Girsanov's theorem**, the central mechanism for changing probability measures. The theorem shows how drift can be removed from stochastic processes by reweighting probabilities via the stochastic exponential, leaving volatility and sample paths unchanged. Both the intuition and rigorous proof are developed.

We then apply Girsanov's theorem to **construct the risk-neutral measure** explicitly. The **market price of risk** emerges as the key quantity governing the change of measure, linking physical dynamics to risk-neutral dynamics. The framework is extended to multi-asset models, stochastic volatility, foreign exchange, and interest rate settings, including changes of numéraire and forward measures.

Finally, the chapter provides a **financial interpretation** of measure change. We clarify the distinction between pricing and hedging, between the physical and risk-neutral worlds, and decompose risk premia. We also examine the limitations of the framework, including incomplete markets, strict local martingales, and situations where measure change fails.

---

## Chapter Roadmap

- **4.1** develops the martingale toolkit (local martingales, stochastic exponential, Novikov/Kazamaki, representation theorem)
- **4.2** connects martingales to no-arbitrage pricing (FTAP, risk-neutral valuation)
- **4.3** introduces Girsanov's theorem (intuition, drift adjustment, statement, proof)
- **4.4** constructs the risk-neutral measure and its variants (market price of risk, numéraire, forward measure)
- **4.5** interprets the framework financially (physical vs risk-neutral, pricing vs hedging, failures)

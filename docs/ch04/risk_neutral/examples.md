...
Under $\mathbb{P}$: $dS_t = \mu S_t\,dt + \sigma S_t\,dW_t^{\mathbb{P}}$ with
constant $\mu$, $\sigma$, $r$.
...
Under $\mathbb{P}$: $dS_t = (\mu - q)S_t\,dt + \sigma S_t\,dW_t^{\mathbb{P}}$
...
No-arbitrage determines $\theta_1 = (\mu - r)/\sqrt{V_t}$, but $\theta_2$ (the
**volatility risk premium**) remains free — any choice satisfying integrability gives
a valid $\mathbb{Q}$. Common specifications include $\theta_2 = 0$ and
$\theta_2 = \lambda\sqrt{V_t}$. Different choices yield different option prices:
...
Under $\mathbb{P}$: $dX_t = \mu_X X_t\,dt + \sigma_X X_t\,dW_t^{\mathbb{P}}$
...
Market price of risk: specified exogenously (e.g., $\theta_t = \lambda$) because
the short rate is not a traded asset.

...
!!! abstract "Construction roadmap"
    ...
    1. **Discount**: form the discounted price $\tilde{S}_t = S_t / B_t$.
    2. **Identify the obstruction**: the drift $(\mu_t - r_t)$ prevents $\tilde{S}_t$ from being a martingale.
    3. **Define the Girsanov kernel**: set $\theta_t = (\mu_t - r_t)/\sigma_t$.
    4. **Apply Girsanov’s theorem**: the tilted measure $\mathbb{Q}$ makes $\tilde{S}_t$ a martingale.
...
$$
dS_t = \mu_t S_t\,dt + \sigma_t S_t\,dW_t^{\mathbb{P}}, \qquad dB_t = r_t B_t\,dt
$$
...
The presence of the drift term $(\mu_t - r_t)$ prevents $\tilde{S}_t$ from being

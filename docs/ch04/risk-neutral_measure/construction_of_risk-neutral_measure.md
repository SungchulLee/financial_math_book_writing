...
!!! abstract "Construction roadmap"
    ...
    1. **Discount**: form the discounted price \(\tilde S_t = S_t / B_t\).
    2. **Identify the obstruction**: the drift \((\mu_t - r_t)\) prevents \(\tilde S_t\) from being a martingale.
    3. **Define the Girsanov kernel**: set \(\theta_t = (\mu_t - r_t)/\sigma_t\).
    4. **Apply Girsanov’s theorem**: the tilted measure \(\mathbb{Q}\) makes \(\tilde S_t\) a martingale.
...
\]
Since the drift term has vanished, the semimartingale decomposition of \(\tilde S_t\) under
\(\mathbb{Q}\) contains no finite-variation (predictable) part beyond the initial value.
A continuous process with this property is a local martingale. Since \(\tilde S_t\) is a
...
\(\mathbb{E}^{\mathbb{Q}}[\tilde S_t \mid \mathcal{F}_s] = \tilde S_s\) for \(s \le t\).

...
In the [unifying framework](unifying_principle_controlling_local_martingales.md) of this section, the martingale representation theorem represents **full control** — every source of randomness can be expressed as a stochastic integral, which means every contingent claim can be hedged.

...
Recall (see [§ Carr–Madan FFT](../../ch09/alternative_fourier/carr_madan_fft.md)): with damping $\alpha > 0$, the modified call has Fourier transform

$$
\hat{c}(u) = \frac{e^{-rT}\varphi(u - i(\alpha+1))}{\alpha^2 + \alpha - u^2 + i(2\alpha+1)u},
$$

inverted on an FFT grid satisfying $\Delta u \cdot \Delta k = 2\pi/N$ (Simpson-weighted, typical $\alpha \in \{0.75, 1.5\}$). Admissibility requires $\alpha + 1 < n^*(T)$ — the critical moment of the SV model.

---

## Lewis Formula

Recall (see [§ Lewis Integration Formula](../../ch09/alternative_fourier/lewis_integration_formula.md)): the symmetric, damping-free representation evaluates $\varphi(u - i/2)$ on the half-line and only requires $\mathbb{E}[\sqrt{S_T}] < \infty$, making it robust for SV models near moment-explosion regimes.

---

## COS Method

Recall (see [§ COS Pricing Formula](../../ch09/cos_method/cos_pricing_formula.md), [§ Cosine Coefficients via CF](../../ch09/cos_method/cosine_coefficients_via_cf.md), [§ Truncation to Finite Domain](../../ch09/cos_method/truncation_to_finite_domain.md)): the COS method expands the density in cosine series on $[a,b]$ chosen from cumulants $a = c_1 - L\sqrt{c_2+\sqrt{c_4}}$, $b = c_1 + L\sqrt{c_2+\sqrt{c_4}}$ ($L \approx 10$–$12$). For SV models the cumulants $c_j$ come from CF derivatives at $u=0$; the affine structure (see [§ Affine Structure](../the_heston_model/affine_structure.md)) makes these explicit for Heston.

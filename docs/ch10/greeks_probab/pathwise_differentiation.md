...
## Pathwise estimators for Black–Scholes Greeks

With $S_T = S\exp((r-\tfrac12\sigma^2)\tau + \sigma\sqrt{\tau}Z)$ and $Z\sim\mathcal{N}(0,1)$, the Jacobians $\partial_S S_T = S_T/S$, $\partial_\sigma S_T = S_T(\sqrt{\tau}Z-\sigma\tau)$, $\partial_r S_T = \tau S_T$ yield, for sufficiently regular $\Phi$,

$$
\Delta = \mathbb{E}\!\left[e^{-r\tau}\Phi'(S_T)\tfrac{S_T}{S}\right],
\quad
\nu = \mathbb{E}\!\left[e^{-r\tau}\Phi'(S_T)S_T(\sqrt{\tau}Z-\sigma\tau)\right],
$$

$$
\rho = \mathbb{E}\!\left[-\tau e^{-r\tau}\Phi(S_T) + e^{-r\tau}\Phi'(S_T)\tau S_T\right].
$$

**Integrability.** $\mathbb{E}[(S_T/S)^2]=e^{\sigma^2\tau}<\infty$ and $|Z|e^{aZ}$ has finite Gaussian moments, so the estimators are well-defined; for Lipschitz call payoffs $\Phi'(S_T)=\mathbf{1}_{S_T>K}$ a.s.

Recall (see [§ Greeks in the Black–Scholes Model](../greeks/greeks_in_black_scholes_model.md)): the closed-form values $\Delta_{\text{call}}=N(d_1)$, $\nu_{\text{call}}=S\sqrt{\tau}N'(d_1)$, $\rho_{\text{call}}=K\tau e^{-r\tau}N(d_2)$.
...
Recall (see [§ Likelihood Ratio and Malliavin Methods](likelihood_ratio_malliavin_methods.md)): the score-function identity $\partial_\theta V = e^{-r\tau}\mathbb{E}[\Phi(S_T)\,\partial_\theta\log p_\theta(S_T)]$ avoids $\Phi'$ and handles kinked or digital payoffs; explicit BS weights for $\Delta,\nu,\Gamma$ are tabulated there.

**Rule of thumb**: pathwise for smooth payoffs, LR for digital/barrier; finite-difference Greeks $(V(\theta+d\theta)-V(\theta-d\theta))/(2d\theta)$ are simple but require step-size tuning to balance $O(d\theta^2)$ bias against $O(\epsilon/d\theta)$ amplified noise.

---

## Beyond Black–Scholes

For Heston ($dS_t=rS_t\,dt+\sqrt{v_t}S_t\,dW_t^{\mathbb{Q}}$), linearity of drift and diffusion in $S$ preserves $\partial S_T/\partial S_0 = S_T/S_0$, so $\Delta = e^{-r\tau}\mathbb{E}^{\mathbb{Q}}[\Phi'(S_T)S_T/S_0]$ retains the BS form; only the Monte Carlo variance grows under stochastic volatility. Pathwise vega in Heston is more involved (sensitivity to $v_0$ propagates through the variance process).

...
??? success "Solution to Exercise 1"
    **Gyongy's theorem** states that for any Ito process $X_t$ driven by possibly multi-factor dynamics (including hidden state variables $Y_t$), there exists a one-dimensional Markov diffusion $\bar{X}_t$ whose drift and diffusion coefficients are conditional expectations of the original coefficients:
    ...
    $$
    \bar{\mu}(x, t) = \mathbb{E}[\mu(X_t, Y_t, t) \mid X_t = x], \quad \bar{\sigma}^2(x, t) = \mathbb{E}[\sigma^2(X_t, Y_t, t) \mid X_t = x]
    $$
...
??? success "Solution to Exercise 2"
    ...
    $$
    \sigma_{\text{loc}}^2(90, 1) \approx 0.06 \implies \sigma_{\text{loc}}(90, 1) \approx \sqrt{0.06} \approx 0.245
    $$

    At $K = 110$: The paths reaching $S_t = 110$ predominantly come from Scenario B, so:

    $$
    \sigma_{\text{loc}}^2(110, 1) \approx 0.03 \implies \sigma_{\text{loc}}(110, 1) \approx \sqrt{0.03} \approx 0.173
    $$
...
??? success "Solution to Exercise 3"
    ...
    $$
    L^2(K, t) = \frac{0.25^2}{0.04} = \frac{0.0625}{0.04} = 1.5625
    $$

    $$
    L(K, t) = \sqrt{1.5625} = 1.25
    $$
...
??? success "Solution to Exercise 5"
    ...
    $$
    p(x, t) = \int p(x, y, t) \, dy
    $$

    By the definition of conditional expectation:

    $$
    \mathbb{E}[\mu(X_t, Y_t, t) \mid X_t = x] = \frac{\int \mu(x, y, t) p(x, y, t) \, dy}{p(x, t)}
    $$

    Therefore:

    $$
    \int \mu(x, y, t) p(x, y, t) \, dy = \mathbb{E}[\mu(X_t, Y_t, t) \mid X_t = x] \cdot p(x, t) = \bar{\mu}(x, t) \cdot p(x, t)
    $$
    ...
    **Why uniqueness guarantees $p = \bar{p}$:** After integrating the joint Fokker-Planck equation over $y$, the marginal density $p(x, t)$ satisfies:
    ...
    $$
    \frac{\partial p}{\partial t} = -\frac{\partial}{\partial x}[\bar{\mu}(x, t) p] + \frac{1}{2}\frac{\partial^2}{\partial x^2}[\bar{\sigma}^2(x, t) p]
    $$

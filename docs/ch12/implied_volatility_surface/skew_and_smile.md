...
Recall (see [§ Breeden-Litzenberger Formula](../model_free_results/breeden_litzenberger_formula.md)): the risk-neutral density follows from $\partial^2 C/\partial K^2$, so the smile encodes distributional moments:

- Variance: $\text{Var}(S_T) \leftrightarrow \sigma_{\text{ATM}}^2 T$
- Skewness: $\text{Skew}(S_T) \propto -\mathcal{S}(T)$, so $\mathcal{S}<0 \Rightarrow$ left-skewed density
- Excess kurtosis: $\text{Kurt}(S_T)-3 \propto \mathcal{C}(T)$, so $\mathcal{C}>0 \Rightarrow$ fat tails
...
Recall (see [§ Stochastic Volatility](../../ch14/index.md)): the **SABR** model $(dF_t = \sigma_t F_t^\beta dW^1$, $d\sigma_t = \nu\sigma_t dW^2$, $\langle W^1,W^2\rangle=\rho t)$ admits a Hagan-style asymptotic implied-volatility expansion in $(\alpha,\beta,\rho,\nu)$, controlling ATM level, backbone, skew, and curvature respectively. Widely used in IR and FX.
...
Recall (see [§ Smile Dynamics](../smile_dynamics/dynamic_consistency.md)): under **sticky-strike** ($\sigma_{\text{IV}}(K,\cdot)$ constant in $S_t$, generated by Dupire local vol) the smile shifts in moneyness as spot moves; under **sticky-delta** ($\sigma_{\text{IV}}(\Delta,\cdot)$ constant, generated by stochastic vol) the smile co-moves with spot in strike space. Empirically, equity markets sit between the two, closer to sticky-delta.
...
Recall (see [§ Wing Asymptotics and Moment Constraints](../asymptotics_of_implied_volatility/wing_asymptotics_and_moment_constraints.md)): **Lee's moment formula** ties the wing growth rate of total variance, $p_{\pm} = \lim_{y\to\pm\infty}\sigma_{\text{IV}}^2(y,T)T/|y|$, to the maximum finite moment of the risk-neutral density via $p_{\pm} = 2/m_{\pm}$. Hence wings cannot be too flat: IV must grow at least as $\sqrt{|y|/T}$ to maintain finite variance.

...
Recall (see [§ Vega, Vanna, Volga](../implied_volatility_sensitivities/vega_and_implied_volatility_sensitivity.md)): ATM vega scales as $\mathcal{V}_{\text{ATM}}\sim S_0\sqrt{T/(2\pi)}$, ATM gamma blows up as $T^{-1/2}$, and ATM volga $\to 0$ for small $T$ because $d_1 d_2 \to -\sigma^2 T/4$. Consequence: ATM options have stable vega; wings carry the volga.

...
??? success "Solution to Exercise 3"
    ...
    $$\Phi = V_0 + \int_0^T \alpha_t\, dS_t$$

    for some predictable process $\alpha_t$ (the stock holdings of the replicating strategy, matching the notation of the [self-financing replication derivation](replication.md)). This is possible because the single Brownian motion generates the entire filtration, and a single risky asset is sufficient to span all risk.

...
This page presents the Python implementation for **Black Scholes Implied Vol**.
...
Educational script demonstrating black scholes implied vol concepts.
"""
...
if __name__ == "__main__":
    ...
    class BlackScholesImpliedVol(BlackScholesBase):
        ...
        def create_synthetic_data(self):
            ...
            strikes = np.linspace(self.S0 * 0.9, self.S0 * 1.1, 10)
            maturities = pd.date_range('2024-01-01', periods=6, freq='M')
            
            synthetic_options = []
            for strike in strikes:
                for i, maturity in enumerate(maturities):
                    ttm = (i + 1) * 0.083  # Months to years
                    
                    # Generate realistic option price with volatility smile
                    moneyness = strike / self.S0
                    base_vol = 0.2 + 0.1 * (moneyness - 1)**2  # U-shaped smile
                    
                    # Simple Black-Scholes approximation with noise
                    intrinsic = max(self.S0 - strike, 0)
                    time_value = np.random.normal(2, 0.5) * np.sqrt(ttm)
                    option_price = max(intrinsic + time_value, 0.1)

...
!!! info "Source"
    **Binomial Models in Finance** by John van der Hoek and Robert J. Elliott, Springer, 2006.
    These notes are used for educational purposes.

## Introduction & No Arbitrage

XII
Contents
13
Interest Rate Models . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 171
...
F.1
An Alternative representation of Function (F.1) . . . . . . . . . . . . . 290
F.2
Imposing Smoothness . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 291
F.3
Unknown Coefficients . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 291
F.4
Observations . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 292
F.5
Determination of Unknown Coefficients . . . . . . . . . . . . . . . . . . . . 293
F.6
Forward Interest Rates . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 295
F.7
Yield Curve . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 296
F.8
Other Issues . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 296
...
currencies, bonds and so on.
Derivative assets are financial investments
...
Definition 1.1 (Arbitrage Opportunity). More precisely, an arbitrage op-
...
We borrow and sell A. This realizes
P0(A)
We buy B; this costs
−P0(B)
...
0
−K
Fig. 1.1. The payoffof a long forward contract.
...
0
K
Fig. 1.2. The payoffof a short forward contract.
...
days from July 22 to expiry, (22 July to 15 August); this is
24
365 = 0.06575
...
22 July until 15 August, we would get
18
250 = 0.072 years. There is another
...
which is a 6.82% profit ( 115
1685 × 100 = 6.82%). Suppose now that the view
...
## Binomial Model for Stock Options
...
that S(1, ↑) ̸= S(1, ↓), but without loss of generality (wlog), we may assume
that S(1, ↑) > S(1, ↓).
The riskless asset
...
.
(2.5)
Note: It is rather crucial that S(1, ↑) ̸= S(1, ↓).
...
the latter case you can sell the asset (at a profit or loss) at a future date.
Step 2
...
X(0) represents the amount to be paid at t = 0 for the asset with payoffX(1)
at t = 1. If X(0) < 0, then −X(0) represents an amount received at t = 0 for
the asset with payoffX(1) at t = 1.
We shall review for this call option why X(0) must equal H0 + H1S0. First
...
In fact
2.25S(0) −(7.2 + X(0)) > 0
where 2.25S(0) is income, 7.2+X(0) payouts. Note that because this difference
...
this trade. The short sale of the borrowed stock was enough to finance the
...
rules out. In efficient markets one assumes that arbitrage opportunities do not
...
This yields a positive profit at t = 0 which is placed deep in your pocket until
...
ances out. Thus, there are no unfunded liabilities at t=1.
In summary, we have again made a profit at t = 0 and have no unfunded
...
model if and only if (iff) a formula of the type (2.13) holds with 0 < π < 1.
Remark 2.7. The author that is credited with the first use of binomial option
...
3 . Starting with nothing, choose H0 = −5
(borrow 5 stocks), H1 = 1 (buy one stock). Then H0 + H1S(0) = 0. At t = 1,
our position will be X(1) ≡−5R+S(1) (meaning sell the stock and repay the
loan). This is 10
9 in the upstate and 1
9 in the down state. So with no start-up
capital we have generated a profit (in both states) by simply trading. This is
...
For any 0 ≤p ≤1, let Ep [X(1)] be defined by
Ep [X(1)] = pX(1, ↑) + (1 −p)X(1, ↓).
(2.14)
...
REπ [X(1)] .
(2.15)
For the asset X we can define the return rX by
rX = X(1) −X(0)
X(0)
,
(2.16)
...
their values are X(1, ↑), Y (1, ↑) (resp., X(1, ↓), Y (1, ↓)). Then define V p
X,Y by
V p
X,Y = Covp (rX, rY )
≡Ep [(Ep [rX] −rX) (Ep [rY ] −rY )]
(2.22)
= Ep [rXrY ] −Ep [rX] Ep [rY ]
(2.23)
Lemma 2.15.
...
result fits well with reality—if you want a higher expected return you must
...
Y,Y
,
which is a regression coefficient for the returns of X onto those of Y . This
...
Y together with (2.24) and (2.25). It is necessary to consider p ̸= π and
...
a profit at t = 0 without any unfunded liabilities at t = 1. Here one ends up
with a profit at t = 1 in all states of the world.
...
At t = 1 in ↓our position is H0R + H1S(1, ↑) =
20
9 ; in other words our
investment gives rise to 5 × 4
3 = 20
3 , which is enough to cover the repurchase
of stock at 40
...
(2.34)
we note an obvious similarity. Here N has the definition
N(x) ≡
1
√
2π
...
R
By the way, K
R = PV (K) ≡PV0(K), the present value at t = 0 of K at t = 1
(PV = Present Value).
...
option, buy a put option, buy one stock, borrow PV (K). The short sale and
...
invest PV (K). The two short sales are enough to cover the call options and
...
at expiry. These are type one arbitrages. These financial contradictions show
...
2.6 Non Arbitrage Inequalities
In the section above we saw the first of these: the call-put parity formula. This
...
expires at time T, whose strike price is K. We also write PVt(K) for the value
...
At time t we short sell one stock, invest PVt(K) in a bank, buy a call option,
...
Therefore arbitrage has been established. This is a financial contradiction and
so (2.45) and hence (2.43) and (2.44) hold.
2
...
ropean counterpart. After all the American call option offers all the privileges
of the European call option and other benefits besides—the right to exercise
...
the same specifications of strike price and exercise price). Pocket the profit.
...
to derive financial conclusions.
...
so again (2.48) holds if S(t) ≤K.
2

32
2 The Binomial Model for Stock Options
Remark 2.30.
1. One consequence of this example is the following: If a stock
...
options (90-day options) for which this condition applies. The financial
...
S(0) + PA(0) −CA(0).
(2.55)
We should be able to check this holds from data in the financial press (other-
...
call option early, as it is more profitable to sell the option than to exercise
...
tion). Suppose there are two calls which are identical except they have different
...
these calls at time t are Cτi(t), for i = 1, 2. We claim
Cτ1(t) ≥Cτ2(t)
(2.57)
...
short the τ2 call and purchase the τ1 call. Pocket the profit. At time T2, the
...
time. So we now have a type one arbitrage opportunity. This is a (financial)
...
payoffgraph for the this call option. The profit graph is the plot of C(T)−C(0)
versus S(T). Draw the profit graph. For what values of S(T) will the profit
be positive ? (This profit ignores the time value of money.)
Exercise 2.40. Repeat Exercise 2.39 but for the (European) put option. The
difference between a put and a call is that with the put you have the right to
...
what values of S(T) will the profit be positive? Explain why the holding of a
...
gards risk). Suppose that XYZ offers and charges customers 4% interest on
deposits or loans, while ABC offers and charges 6% interest. Seeing this sit-
uation, how could you make a riskless profit without using any of your own
...
not equally rated, what possible reason could you give for the difference in
interest rates?
...
butterfly spread is the combination C1 −2C2 + C3 with K2 = 1
2(K1 + K3).
Graph C(T) against S(T). Show that C2(0) <
1
2(C1(0) + C3(0)). Discuss
...
C(0) −P(0) = S(0) −PV (K),
where PV (K) = K/R. With the choices S(0) = $10.50, K = $10.00, C(0) =
...
against strike prices and seek the least squares fit line. Find its slope m and
...
formula. The CRR paper uses u = exp(σ
√
∆t) where σ is the volatility and
∆t is the time interval between t = 0 and t = 1, which we will take as
31/365 = 0.08493 and so
√
∆t ≈0.291431. It may therefore be of interest to
plot ln u versus σ, get the line of best fit, and see if the estimated slope is
...
0.6800
0.6765
In making your line of least squares fit, use wi = 1/5 = 0.2 for i = 1, 2, 3, 4, 5.
...
line of best fit through them. The model will be
yi = mxi + c + ϵi
(2.58)

2.7 Exercises
37
for each i = 1, 2, . . . , N. Here ϵi denotes an error for each i.
The least squares fit line is that line (or choice of m, c) so that
N
...
(2.59)
is minimal.
In finance we may have ni measurements all at (xi, yi) and ni may not be
...
wix2
i .
If N > 1, then x2 ̸= x2, so we never divide by zero! In fact
x2 −x2 =
N
...
n3 = 941, n4 = 969, n5 = 268. Then M = 2896 and so w1 =
56
2896, w2 =
662
2896,
...
268
2896. Then
x = 56 × 10.50 + 662 × 11.00 + 941 × 11.50 + 969 × 12.00 + 268 × 12.50
2896
= 11.62620856
y = 56 × 1.36 + 662 × 0.95 + 941 × 0.62 + 969 × 0.38 + 268 × 0.20
2896
= 0.590573204
x2 = 56 × 10.502 + 662 × 11.002 + . . . + 969 × 12.002 + 268 × 12.502
2896
= 135.4054731
xy = 56 × 1.36 × 10.50 + 662 × 0.95 × 11.00 + . . . + 268 × 0.20 × 12.50
2896
= 6.73879489
...
know some statistics about the errors, then you can discuss the confidence
...
(2.63)
for each i = 1, 2, . . . , N. Here ϵi denotes an error for each i.
The (weighted) least squares fit line is that line (or choice of m) making
N
...
## Binomial Model for Other Contracts

3
The Binomial Model for Other Contracts
We now discuss the simple one-step binomial model applied to other fi-
nancial contracts.
...
Definition 3.1 (Forward Contract). A (long/short) forward contract is an
...
The payoffof a long forward contract is then S(T) −F. (Buy at F and
sell at S(T)). This payoffmay be positive, negative or zero depending on the
...
The net position is F −S(0)R > 0, which is a clear profit. So, with no net
outlay at time t = 0, one can generate a positive profit at time T. This is an
...
a clear profit. Again with no net outlay at time t = 0, one can generate a
positive profit at time T. This is an arbitrage and a violation of the axiom.2

3.2 Contingent Premium Options
43
Remark 3.2. We should discuss the market for forward contracts. Why do
players enter these contracts? There are again different reasons depending on
...
At expiry T of the forward contract either party could be worse offcompared
with not having a forward contract. Suppose that S(T) > F. Then the farmer
must sell the wheat for F and not for the higher price S(T), and so suffer
...
to the quality agreed in the forward contract. Again the farmer may suffer a
loss in order to honor the forward contract.
A more complete discussion of forwards and futures is given in Duffie [26] and
in many other books.
3.2 Contingent Premium Options
...
USD (inverse, market convention) is equivalent to 1 USD =
1
0.72 = 1.389 CAD
...
given W(1) in CAD, find W(0) in CAD.
...
W(1, ↓) = H0Rd + H1RfX(1, ↓)
(3.3b)
and if X(1, ↑) ̸= X(1, ↓), we have a unique solution for H0 and H1:
H1 =
1
Rf
W(1, ↑) −W(1, ↓)
...
Definition 3.3 (Arrow-Debreu Securities). These are contingent claims
which equal 1 in a single future state and are 0 otherwise.
Let W ↑and W ↓be the (two) Arrow-Debreu securities defined by
W ↑: W ↑(1, ↑) = 1,
W ↑(1, ↓) = 0
(3.8a)
W ↓: W ↓(1, ↑) = 0,
...
the payoffs of any other security at t = 1.
For W ↑:
1 = H0Rd + H1RfX(1, ↑)
0 = H0Rd + H1RfX(1, ↓)
implies
...
Rd
.
As X(1, ↓) < X(1, ↑) we must also have Rd
Rf X(0) > X(1, ↓).
...
Rf
u −d .
Remark 3.4. In the financial press some exchange rates are quoted in the Euro-
...
obligation) to buy F USD at a fixed exchange rate K at time T. We need to
...
for F · X(T) CAD, making a profit of F · X(T) −F · K = F [X(T) −K]. If
...
to yield a profit F[K −X(T)]. If, on the other hand, K ≤X(T), then let the
...
obligation to buy a specified amount F USD at a specified exchange rate
K at time T, no matter what the actual exchange rate is at time T. No
payment is made initially, and the profit at time T is F [X(T) −K], which
can be negative. If the present value of this profit is zero, then K is called the
...
invest in CAD at interest rate rd, convert back, and obtain a riskless profit.
...
(a) Suppose (if possible) that K > Rd
Rf X(0), where K is the T-forward rate.
...
Rf
· Rd + K > 0 CAD
and so we have a certain profit at time t = T with no net outlay at time t = 0.
This is an arbitrage opportunity in contradiction to the axiom.
(b) Also, K < Rd
Rf X(0) leads to a similar contradiction.
...
Rf
· Rd −K > 0 CAD.
Again there is a certain profit at time t = T with no net outlay at time t = 0.
This is again an arbitrage opportunity in contradiction to the axiom.
2
...

(3.15)
and everything is expressed in USD. So 
W(0) is the value in USD of an asset
whose values at t = 1 are 
W(1, ↑) in ↑and 
W(1, ↓) in ↓.
...
F USD). The exchange rate is
1
K from the U.S. point of view. This is a put.
So
...

.
We have assumed that X(1, ↓) < K < X(1, ↑) so Y (1, ↑) < 1
K < Y (1, ↓). Note
that Y (1, ↑) < Y (1, ↓).
We now claim that
πf
...
values of known future cash flows. For example, if there are known (they
are not random) cash flows
C1, C2, C2, . . . , Cm
...
π = P(0, T)R −P(1, T, ↓)
P(1, T, ↑) −P(1, T, ↓).
This says that under no arbitrage, the definition of π in (3.22a) above, is
...
this framework. Consider quantities R, R(1, ↑), and R(1, ↓) defined as follows.
...
R(1, ↓) in an arbitrary way, but there are still infinitely many choices in (3.25)
...
58
3 The Binomial Model for Other Contracts
from which we solve for R(1, ↑) and then R(1, ↓) = 1
kR(1, ↑).
If we write
R = 1 + r(0)
R(1, ↑) = 1 + r(1, ↑)
...
(spread) of the interest rates. The larger the value of k the larger the difference
between R(1, ↑) and R(1, ↓) and, hence, the larger the difference between
...
From (a) and (b), we have established the result.
2
...
C(0) −P(0) −P(0, T) + K · P(0, 1) > 0,
(3.31)
then at time t = 0, short sell one call, buy a put, buy a T-zero, borrow
K · P(0, 1) (short sell K 1-zeros). Pocket the net profit.
...
month or
1
12 years. We also know that P(0, 2) = 0.9950, K = 0.997. We wish
to find C(0) if
C(1) = (P(1, 2) −K)+ .
Now
R = 1 + 0.03
...
multi-period models.
Let us note an interesting difference between stock prices and interest rates. If
...
the basic model of the previous chapter). For the risky asset we have ˜S with

62
3 The Binomial Model for Other Contracts
˜S(0) = 0, ˜S(1, ↑) > 0 and ˜S(1, ↓) < 0. The riskless asset is the same as before.
Now derive again (2.12) but using ˜S in place of S. Give an example for ˜S.
...
T with delivery price K, draw a graph of your profit at time T. Explain how
...
contracts. Discuss the similarities and differences, including a discussion of
differences in risks taken with these two strategies.
...
C = π
R(S(↑) −S)
find π, S(↑) and S(↓) in terms of S, R, F, C. Hint: Show that
S(↑) = S[R C + S]
F −R C
(3.32)
...
currencies are 5.12%, 5.74% and 0.65%, respectively. So Rd = 1 + 0.0512
12
≈
...
3 The Binomial Model for Other Contracts
Exercise 3.26.
1. Let πa (the American π) be given by (3.35), and πj (the
Japanese π) be given the same formula but with all a replaced by j. Define
...
By P (n)
j
(T) we will mean the value at time t = n in state j of $1 at time
...
in place of S and the definition of π(n, j) involves a Rd(n, j) and an Rf(n, j),
so
...
For Bonds
We may choose (any) T ≥2 and define
π(n, j) =
P n
j (T) · R(n, j) −P n+1
j
...
0 < π(n, j) < 1.
4.4 Generalized Backward Induction Pricing Formula
In any application (with the appropriate definitions of π) we have the back-
ward induction pricing formula.
W(n, j) =
1
R(n, j) [π(n, j)W(n + 1, j + 1) + (1 −π(n, j))W(n + 1, j)] .
(4.4)
...
each n < N and j = 0, 1, 2, . . . , n. In particular we can find W(0, 0).
...
(4.6d)
With S(0, 0) = S
S(n, j) = S(0, 0)ujdn−j ≡Sujdn−j
and
W(n, j) = 1
R [πW(n + 1, j + 1) + (1 −π)W(n + 1, j)] .
...
l ≡
k!
l!(k −l)! = k(k −1)(k −2) · · · (k −l + 1)
l(l −1)(l −2) · · · 3 · 2 · 1
for 0 ≤l ≤k,
is one of the binomial coefficients. There are various proofs of (4.7) and (4.8).
...
R2

π2W(2, 2) + 2π(1 −π)W(2, 1) + (1 −π)2W(2, 0)

...
k=0
λ(n −1, k)
R(n −1, k) [π(n −1, k)W(n, k + 1)+(1 −π(n −1, k)W(n, k)] .
We now compare the coefficient of W(n, j) in the first and last lines.
4.8 Application to CRR Model
...
R
λ(n −1, j) + π
R λ(n −1, j −1) if 1 ≤j < n
1 −π
R
λ(n −1, 0)
if j = 0.
(4.14)
We now find the solution of (4.14). It is provided by the following lemma.
Lemma 4.2.
λ(n, j) = Cn
j πj(1 −π)n−jR−n,
(4.15)
...
j =
n!
j!(n−j)! is the binomial coefficient. (For a discussion of binomial
...
More formally, let M = N −n and V (k, l) ≡W(k + n, l + j), then V satisfies
...
l
with u
d > 1, so SuldN−l is an increasing function of l. Therefore, a exists.
We can write
...
.
(4.18)
It is now convenient to make a definition.
Definition 4.5. For 0 < p < 1, set
Φ[a; n, p] =
n
...
In the one-step binomial model let B = S
S ≡1 and S = B
S . Then the stock
...
76
4 Multiperiod Binomial Models
B(0) = 1, B(1, ↑) = 1, B(1, ↓) = 1. Thus R = 1. S(0) =
1
S(0), S(1, ↑) =
R
u S(0) ≡uS(0), S(1, ↓) = R
...
numeraire we have B(n, j) = S(n,j)
S(n,j) ≡1, S(n, j) = B(n,j)
S(n,j) ≡
Rn
S(n,j), and again
with the CRR model we have π(n, j) ≡π′ for all (n, j).
...
nomial model. The CRR paper is one of the most cited papers in the finance
literature.
4.11 Exercises
...
modify inputs for S, r, K = 90 and produce a modified spreadsheet. Now
compute the value of this same American put option when σ = 20%, 25%,
and find the value of σ correct to four significant figures so that the price of
the American put option is $5.00.
Calculate the European put option for the same data and various choices of σ.
Also compute the early exercise premia in these cases (the difference between
the American put value and the European put value).
Exercise 4.12 (The chooser option).
This is also called the As You
...
the same strike price) at t = T. To value the chooser option, we first compute
...
if a significant event (company takeover, etc.) were to occur at t = S. You
...
come under the title of compound options and were first studied by Geske
...
The equity in a levered firm can be viewed as a call option on the
value of the firm. To see this, suppose that the value of the firm is
...
is V −A. Thus, the equity in the firm is a European call option on
V with maturity T ∗and exercise price A. An option on a stock of
the firm that expires prior to T ∗can be regarded as an option on an
option on V .
Exercise 4.16 (The binary option). The binary call option is a European
style option with payoffat expiry W(N, j) = B > 0 when S(N, j) > K and
nothing otherwise. The binary put option has payoffat expiry W(N, j) = B >
...
80
4 Multiperiod Binomial Models
options. As the demand is there, new financial products are designed. Such
...
and sell it to you for K as agreed. I would suffer a loss S(T) −K, but in the
...
also called self-financing dynamic hedging.
...
Section 2.3, [see equation (2.2)]. We define H0(n, j) and H1(n, j) for each
(n, j) so that

82
5 Hedging
W(n + 1, j + 1) = H0(n, j)R(n, j) + H1(n, j)S(n + 1, j + 1)
(5.1)
W(n + 1, j) = H0(n, j)R(n, j) + H1(n, j)S(n + 1, j).
(5.2)
In fact, (5.1) generalizes (2.6), (5.2) generalizes (2.3a) and (5.2) generalizes
(2.3b). We then have from (5.1) and (5.2) the formulae
H1(n, j) = W(n + 1, j + 1) −W(n + 1, j)
S(n + 1, j + 1) −S(n + 1, j)
(5.3)
H0(n, j) = W(n, j) −H1(n, j)S(n, j).
(5.4)
...
this name.
Observe that
W(n + 1, j + 1) −H1(n, j)S(n + 1, j + 1) = H0(n, j)R(n, j)
(5.5)
W(n + 1, j) −H1(n, j)S(n + 1, j) = H0(n, j)R(n, j).
(5.6)
...
Also note that
W(n, j) = H0(n, j) + H1(n, j)S(n, j)
(5.7)
= H0(n −1, j −1)R(n −1, j −1) + H1(n −1, j −1)S(n, j)
(5.8)
= H0(n −1, j)R(n −1, j) + H1(n −1, j)S(n, j).
(5.9)
This implies
...
The conditions (5.7)–(5.9) are termed self-financing, that is, we reduce our
...
final column can be entered and the earlier columns completed using (4.4),
(5.3) and (5.4). We show how to use the calculations.
Suppose that the market price of the call were $36.00 (rather than $34.0796
as in the model). Here is how one can make an arbitrage profit under the
...
(H1(0, 0) = 0.7190). This realizes a profit $1.9204, which we pocket.
...
sent no net inflow or outflow of money. That is, our portfolio is self-financing.
...
net inflow or out flow of money. This is again self-financing.
...
the hedging procedure just described. At expiry you produce a payoffof the
...
cost (outflow of funds) and “in” to indicate receipts (in flow of funds) to cash
...
G(n, j) for each n < N,
j = 0, 1, . . . , n, and in particular we want to find
G(0, 0).
...
given by
M(n + 1, j + 1) = M(n, j)R(n, j) + [G(n + 1, j + 1) −G(n, j)]
(6.4)
M(n + 1, j) = M(n, j)R(n, j) + [G(n + 1, j) −G(n, j)] ,
(6.5)
and for L, on the short side
L(n + 1, j + 1) = L(n, j)R(n, j) −[G(n + 1, j + 1) −G(n, j)]
(6.6)
L(n + 1, j) = L(n, j)R(n, j) −[G(n + 1, j) −G(n, j)] .
(6.7)
...
is called resettlement. A good reference for futures markets is Duffie [26].
Computing Futures Prices
Let π(n, j) be the risk-neutral up probability (see Chapter 4 ). Then using
risk-neutral pricing
M(n, j) = π(n, j)
R(n, j)M(n + 1, j + 1) + 1 −π(n, j)
R(n, j)
M(n + 1, j)
= π(n, j)
R(n, j) [M(n, j)R(n, j) + (G(n + 1, j + 1) −G(n, j))]
+ 1 −π(n, j)
R(n, j)
[M(n, j)R(n, j) + (G(n + 1, j) −G(n, j))]
= M(n, j) + π(n, j)
R(n, j) [G(n + 1, j + 1) −G(n, j)]
+ 1 −π(n, j)
R(n, j)
[G(n + 1, j) −G(n, j)] .
This implies that
...
We then calculate
π(n, j) = S(n, j)R(n, j) −S(n + 1, j)
S(n + 1, j + 1) −S(n + 1, j)
P(n, j) ≡P n
j (N −n)
= π(n, j)
R(n, j)P(n + 1, j + 1) + 1 −π(n, j)
R(n, j)
P(n + 1, j)
F(n, j) = S(n, j)
P(n, j)
...
F(0, 0) = $107.36
G(0, 0) = $107.12.
This difference is not due to arithmetic errors.
...
We shall first analyse the long side. The default could have occurred because
...
market is a common feature of many financial markets as a guarantee
against default by either party. This arrangement is regarded as one of the
great financial innovations of the twentieth century.
...
j (N −n)
where P n
j (N −n) is the value at (n, j) of $1 at time t = N, so
P n
j (N −n) =
1
R(n) ·
...

1
R(n) [π(n, j)S(n + 1, j + 1) + (1 −π(n, j))S(n + 1, j)]

...

= π(n, j)F(n + 1, j + 1) + (1 −π(n, j))F(n + 1, j).
(6.11)
That is
F(n, j) = π(n, j)F(n + 1, j + 1) + (1 −π(n, j))F(n + 1, j),
(6.12)
which is the same backward recursion formula as (6.8) for G. From (6.10), we
conclude that F(n, j) = G(n, j) for all n < N and j = 0, 1, . . . , n.
2
6.3 Exercises
...
Exercise 6.9. If F(0, 0) > G(0, 0), is it possible to make an arbitrage profit
...
We shall now exhibit one of the flexible aspects of the binomial pricing
...
V (n, j) ≥(K −S(n, j))+.
We can compute V by a simple modification of the backwardization formula.
At node (n, j) we first calculate
W(n, j) = π(n, j)V (n + 1, j + 1) + (1 −π(n, j))V (n + 1, j)
R(n, j)
and compare W(n, j) with (K −S(n, j))+. If W(n, j) > (K −S(n, j))+, then
we do not exercise the option as it is more profitable to hold the option than
...
$18.51. The difference $4.33 = $(18.51 – 14.18) is called the early-exercise
premium.
...
clear that, as the barrier option offers less protection than the plain vanilla
...
better fulfill the user’s needs. For example, you wish to forgo insurance when
you do not think it will be needed. Perhaps insurance companies could offer
cheaper insurance with a product that offers no insurance against fires in
winter nor against floods in summer.
...
continuous monitoring, but this may be difficult to implement. The type of
monitoring will have an effect on the barrier option price. However, we leave
this subject to the interested reader. In our binomial models, we shall monitor
at nodes in the tree.
Specifications. For a barrier option we need to specify which of the eight
types it is, the strike price, the barrier, the expiry date and, if needed, the
face value.
Many banks offer barrier options and employ quants to price them. You need
...
except for some modifications. Let V denote the value of this option. Then
V (N, j) =
...
We then use the backwardization formula to find all other values of V (n, j),
where S(n, j) < B.
...
option prices.
Example 7.5 (Up and in call option).
You may be an investor/speculator
...
102
7 American and Exotic Option Pricing
We first find the value of V at the barrier nodes (the ones on or just above
...
C(N, j) = (S(N, j) −K)+
C(n, j) = π(n, j)C(n + 1, j + 1) + (1 −π(n, j))C(n + 1, j)
R(n, j)
,
...
here, not give financial advice.
...
Example 7.7 (Down-and-in put).
Consider now a company that expects to
...
be 6 million USD worse off. We could repeat the discussion of Example 7.6,
...
the cheaper option.
Example 7.10 (Up-and-out call). Consider a modification of the example of
...
V (n, j) =
1
1.0125 [0.5378V (n + 1, j + 1) + 0.4622V (n + 1, j)]
(7.5)
...
the Black and Scholes framework is given in Chesney and Louberg´e [12] and
also in Dana and Jeanblanc [20].
Exercise 7.19 (Partial barrier option).
Consider Example 7.4. Repeat
...
We shall confine our examples to ones with small n (say n is 4 or 5). Those
...
0012, 2 = 0102, 3 = 0112, and so on, where the suffix 2 means a positive integer
M is written in base 2 as M = (apap−1 . . . a0) = ap2p +ap−12p−1 +· · ·+a020.
...
u(n, j) −d(n, j)
(8.3)
W(n, j) = π(n, j)W(n + 1, j1) + (1 −π(n, j))W(n + 1, j0)
R(n, j)
(8.4)
where (8.4) would be suitably modified for American style options.
8.2 Asian Options
For the European call Asian option, the payoffat expiry is given by
W(N, j) = (A(N, j) −K)+
(8.5)
and for the European put Asian option, the payoffat expiry is given by
W(N, j) = (K −A(N, j))+ .
(8.6)
...
Hence the payoffin (8.5) is path-dependent.
We can write
A(N, j) = S(0, 0) + S(1, j1) + . . . + S(n, jn) + . . . + S(N, j)
N + 1
(8.7)
for any admissible path through the tree
(0, 0) →(1, j1) →. . . →(n, jn) →. . . →(N, j).
With the notation of the previous section, the state label jn is obtained from
the state label of (N, j) by truncation to the first n + 1 digits.
...
as follows:
A(0, 0) = S(0, 0)
A(n, j1) = n · A(n −1, j) + S(n, j1)
n + 1
A(n, j0) = n · A(n −1, j) + S(n, j0)
n + 1
.
Application
...
Asian Call(0) −Asian Put(0) = S(0) −PV0(K),
(8.8)
...
put option.
Variants
Many variants can be made for the payoffof the Asian option. Here are some:
8.3 Floating Strike Options
W(N, j) = (A(N, j) −S(N, j))+
(8.9)
W(N, j) = (S(N, j) −A(N, j))+.
(8.10)
We could also replace A by alternative definitions. Here are examples that
have been used:
A(N, j) = S(k, jk) + S(k + 1, jk+1) + . . . + S(N, j)
N −k + 1
(8.11)
A(N, j) = S(N, j) + S(N −1, jN−1) + . . . + S(N −k + 1, jN−k+1)
k
.(8.12)
In (8.12) the usual S in the payoffof a call is replaced by the average of
...
113
8.4 Lookback Options
There are two types. The payoffof the put is
W(N, j) = M(N, j) −S(N, j)
(8.13)
where
M(N, j) ≡M(N, jN) = max{S(n, jn) : n = 0, 1, . . . , N}.
Note that this path-dependant payoffcan never be negative. This is a very
...
The payoffof the call is
W(N, j) = S(N, j) −m(N, j),
(8.14)
where
m(N, j) ≡m(N, jN) = min{S(n, jn) : n = 0, 1, . . . , N}.
So we note that payoffcan never be negative. This is also a very desirable
...
Some investment houses offer products which try to attract customers by
having aspects of the lookback options.
Variants
There are many variants. Here are some other payoffs:
W(N, j) = (M(N, j) −K)+
(8.15)
W(N, j) = (K −m(N, j))+,
(8.16)
...
Example 8.2. The first example is the pricing of the the lookback put with the
...
8.5 More on Average Rate Options
115
options. The key reference for this method was given earlier in the first section
of this chapter.
We shall focus our discussion on the example given in Section 8.2, which was
the European call Asian option with fixed strike price.
The Hull and White Method
Step 1
At each node (n, j) we compute four numbers,
A1(n, j) < A2(n, j) < A3(n, j) < A4(n, j)
(8.17)
defined as follows. A1(n, j) represents the smallest value of the average of
...
Example 8.4. We choose S(0, 0) = 100, σ = 0.2, T = 0.5, N = 5, ∆=
T
N ,
u = exp(σ
√
...
Ai(N, j) −K
+
for i = 1, 2, 3, 4 and j = 0, 1, . . . , N.
We shall select K = 100.
Step 3
We now begin the backwardization. Let us show how to compute Ci(n, j) for
i = 1, 2, 3, 4.
Compute first
M(↑) =
1
...

= 1
6 [5 × 98.5509 + 106.5288] = 99.8806 ∈[97.0649, 101.4277].
Continuing, we express
M(↑) = λAk(n + 1, j + 1) + (1 −λ)Ak+1(n + 1, j + 1)
with 0 ≤λ ≤1 whenever
M(↑) ∈

Ak(n + 1, j + 1), Ak+1(n + 1, j + 1)

.
For C2(4, 2), M(↑) ∈

...
and
Ci(n, j)(↑) = λCk(n + 1, j + 1) + (1 −λ)Ck+1(n + 1, j + 1)
= Ck+1(n + 1, j + 1)
+ λ

Ck(n + 1, j + 1) −Ck+1(n + 1, j + 1)

.
We now compute Ci(n, j)(↓) in a similar manner:
M(↓) =
1
n + 2

...
Al+1(n + 1, j) −Al(n + 1, j)
and then set
Ci(n, j)(↓) = λCl(n + 1, j) + (1 −λ)Cl+1(n + 1, j)
= Cl+1(n + 1, j) + λ

Cl(n + 1, j) −Cl+1(n + 1, j)

...

= 1
6 [5 × 98.5509 + 93.8713] = 97.7710 ∈[95.2115, 99.3068]

118
8 Path-Dependent Options
M(↓) = λ95.2115 + (1 −λ)99.3068
λ = 0.3750
C2(4, 2)(↓) = λC2(5, 2) + (1 −λ)C3(5, 2)
...
C2(4, 2) =
1
1.0050 [0.0.5238 × 0.9214 + 0.4762 × 0.0000] = 0.4802.
This procedure is repeated to compute Ci(n, j) for all choices of i, n, j.
The final answer by this method is $4.58. We would not expect this answer to
...
option determine by a finite number of ladder levels
L1 < L2 < ... < Lk.
To determine its payoffW(N, j) at t = N, we first compute the maximum
process value M(N, j) defined in Section 8.4. If M(N, j) < L1, then
W(N, j) = max (S(N, j) −K, 0) ,
...
Consider a 10-step binomial tree with S(0, 0) = 100, u = exp(σ
√
∆t), d = 1/u,
...
H1 = C(1, 1) −C(1, 0)
S(1, 1) −S(1, 0) = 14.579 −7.2968
104.86 −95.367 = 0.7671,
(9.6)
...
is the (continuous time) Black and Scholes partial differential equation (PDE)
for option pricing. This could be written more precisely. Solve the boundary
value problem for u = u(t, x):
ut(t, x) + rxux(t, x) + 1
2σ2uxx(t, x) −ru(t, x) = 0
for x > 0 and t > 0 and with final condition u(T, x) = (x −K)+. Then the
...
will be good to recall some basic definitions about dividends.
...
close to that amount). Perhaps you should look for this in the financial press
...
dividends, giving an arbitrage profit.
...
so that
1 = H0(n, j)R(n, j) + H1(n, j) [S(n + 1, j + 1) −f(n, j)]
(10.2)
0 = H0(n, j)R(n, j) + H1(n, j) [S(n + 1, j) −f(n, j)] .
(10.3)
Multiply (10.2) by S(n + 1, j) −f(n, j) and (10.3) by S(n + 1, j + 1) −f(n, j)
and subtracting the results leads to
H0(n, j) =
f(n, j) −S(n + 1, j)
R(n, j)(S(n + 1, j + 1) −S(n + 1, j)) = p(n, j)
R(n, j)
...
H0(n, j) = π(n, j)
R(n, j)
and the result p(n, j) = π(n, j) follows.
2
Lemma 10.2.
f(k, j) = S(k, j) · R(k, j) −Dk+1.
(10.4)
...
f(n, j) = (1 −δ(n + 1))S(n, j)R(n, j) ≡(1 −δ)S(n, j)R(n, j).
(10.5)
This follows as
0 = π(n, j)
R(n, j) [(1 −δ)u(n, j)S(n, j) −f(n, j)]
+ 1 −π(n, j)
R(n, j)
[(1 −δ)d(n, j)S(n, j) −f(n, j)]
and
S(n, j) = π(n, j)
R(n, j) [(1 −δ)u(n, j)S(n, j) + δu(n, j)S(n, j)]
+1 −π(n, j)
R(n, j)
[(1 −δ)d(n, j)S(n, j) + δd(n, j)S(n, j)] .
Consider the CRR model with cum-dividend prices S(n, j) = S(0, 0)ujdn−j.
Here u = exp(σ
√
∆t) = 1/d, R = exp(r∆t), π = (R −d)/(u −d).
Then the ex-dividend prices form a recombining tree of prices. These ex-
dividend prices are
˜S(n, j) = S(0, 0)(1 −δ(1)) . . . (1 −δ(n))ujdn−j,
(where some of the δ(k) could be 0). By (10.5), π is the risk-neutral up prob-
ability for the ex-dividend tree as well.
We suppose the ex-dividend price is modelled by ˜S:
...
π ˜S(n + 1, j + 1) + (1 −π) ˜S(n + 1, j) = (1 −δ(n + 1)) ˜S(n, j)R(n, j). (10.8)
...
(1 −δ(1)) · · · (1 −δ(n + 1))

πuj+1dn+1−j−1 + (1 −π)ujdn+1−j
= S(0, 0)(1 −δ(1)) · · · (1 −δ(n + 1))ujdn−j [πu + (1 −π)d]
= S(0, 0)(1 −δ(1)) · · · (1 −δ(n + 1))ujdn−jR
≡˜S(n, j)(1 −δ(n + 1))R(n, j),
as required.
...
option written on a dividend paying stock S. Let ˜S denote the ex-dividend
price process of S. Then
V (n, j) ≥π(n, j)V (n + 1, j + 1) + (1 −π(n, j))V (n + 1, j)
R(n, j)
≥π(n, j)[ ˜S(n + 1, j + 1) −K]+ + (1 −π(n, j))[ ˜S(n + 1, j) −K]+
R(n, j)
≥

π(n, j) ˜S(n + 1, j + 1) + (1 −π(n, j)) ˜S(n + 1, j) −K
+
R(n, j)
...
Define
˜S(n, j) = (S(0, 0) −D0) ujdn−j + Dn.
Then
˜S(0, 0) = S(0, 0)
π ˜S(n + 1, j + 1) + (1 −π) ˜S(n + 1, j) = R ˜S(n, j) −Dn+1.
By Lemma 10.2, the tree of prices ˜S behaves like ex-dividend prices and π
...
10.3 Binomial Trees with Known Dollar Dividends
133
π ˜S(n + 1, j + 1) + (1 −π) ˜S(n + 1, j) = ˜S(n, j)R(n, j) −Dn+1,
(10.11)
...
π

(S(0, 0) −D0) uj+1dn+1−j−1 + Dn+1

+ (1 −π)

(S(0, 0) −D0) ujdn+1−j + Dn+1

= (S(0, 0) −D0) ujdn−j [πu + (1 −π)d] + Dn+1
= (S(0, 0) −D0) ujdn−jR + Dn+1
= R

˜S(n, j) −Dn

...
there are no dividends and ˜S(n, j) for the ex-dividend stock price when there
are dividends. When there are no dividends, the formulas for S and ˜S clearly
coincide.
...
Thus, with ∆t = 1, R = exp(0.01), u = exp(σ
√
δt) = exp(0.15), d = 1/u.
D0 = D2
R2 = 0.81873075
...
call prices differ with K = 80. They are now $32.72 and $31.76, respectively.
...
We do not digress to prove these claims, but note that in this last modification
...
call option written of a dividend-paying stock S. Let ˜S denote the ex-dividend
price process of S. Then
V (n, j) ≥

˜S(n, j) −Dn+1 + K
...
[13] and Barle and Cakici [3]. The first two papers are brought together in
...
In the first instance we shall focus on European put and call market prices.
...
The first sections deal with the original Derman and Kani constructions and
...
practice the market provides only a finite number of option prices, and hence
a finite number of implied volatility values (for various strike prices and times
to maturity). An implied volatility surface is then constructed by interpolat-
ing or extrapolating from this finite number of values. Any call and put prices
...
assume that various call and put prices with different strike prices and times
...
V put(n −1, j) =
1
R(n −1, j)(1 −π(n −1, j)) [K −S(n, j)] .
(11.1)
Now we use
1 −π(n −1, j) = S(n, j + 1) −R(n −1, j)K
S(n, j + 1) −S(n, j)
.
(11.2)
...
137
S(n, j) =
V put(n −1, j)S(n, j + 1) + K

K −S(n,j+1)
R(n−1,j)

V put(n −1, j) + K −S(n,j+1)
R(n−1,j)
,
(11.3)
or, recalling K = S(n −1, j),
S(n, j) =
V put(n −1, j)S(n, j + 1) + S(n −1, j)

S(n −1, j) −S(n,j+1)
R(n−1,j)

V put(n −1, j) + S(n −1, j) −S(n,j+1)
R(n−1,j)
.
(11.4)
Case 2
...
λ(n −1, k)C(n,K)
E
(n −1, k) + λ(n −1, j)V call(n −1, j)
= ΣC(n −1, j) + λ(n −1, j)V call(n −1, j).
Then
V call(n −1, j) = C(n,S(n−1,j))
E
−ΣC(n −1, j)
λ(n −1, j)
(11.20)
...
−K
10 + 25.5 if K ≥110.
This last statement means that if we want P (n,K)
E
we use the Black and Scholes
...
and C2,S(1,1)
E
. These are computed using the Black and
...
ΣP (1, 0) = 0
ΣP (1, 1) = λ(1, 0)
R(1, 0) [S(1, 1) −S(1, 0)R(1, 0)] = 6.1969
ΣC(1, 1) = 0
ΣC(1, 0) = λ(1, 1)
R(1, 1) [S(1, 1)R(1, 1) −S(1, 0)] = 16.8406.
Then
V put(1, 0) = P 2,S(1,0)
E
−ΣP (1, 0)
...
So, by (11.4) and (11.8),
S(2, 0) =
V put(1, 0)S(2, 1) + S(1, 0)

S(1, 0) −S(2,1)
R(1,0)

V put(1, 0) + S(1, 0) −S(2,1)
R(1,0)
= 56.8625
and
S(2, 2) =
V call(1, 1)S(2, 1) + S(1, 1)

S(2,1)
R(1,1) −S(1, 1)
...
π(1, 0) = R(1, 0)S(1, 0) −S(2, 0)
S(2, 1) −S(2, 0)
= 0.8296
π(1, 1) = R(1, 1)S(1, 1) −S(2, 1)
S(2, 2) −S(2, 1)
= 0.6171
λ(2, 0) = 1 −π(1, 0)
R(1, 0
...
using σ = 0.15
ΣP (2, 1) = λ(2, 0)
R(2, 0) [S(2, 1) −S(2, 0)R(2, 0)]
= λ(2, 0)
R(2, 0) [100 −S(2, 0)R(2, 0)] = 1.9398
V put(2, 1) = 4.7106
u = 1.151269614
S(3, 2) = 115.1269
S(3, 1) = 86.8606,
...
f(n, i) = S(n, i) exp[(r −q)∆t]
(11.27)
= π(n, i)S(n + 1, i + 1) + (1 −π(n, i))S(n + 1, i)
(11.28)
...
P(K, n) for puts is defined in a similar way. Clearly C(K, n) ≡Cn,K
E
(0, 0) of
...
f(n −1, j) < S(n, j + 1) < f(n −1, j + 1)
(11.30)
for j = 0, 1, 2, ..., n −1, and then
π(n −1, j) = f(n −1, j) −S(n, j)
S(n, j + 1) −S(n, j)
(11.31)
satisfies 0 < π(n −1, j) < 1 for j = 0, 1, 2, ..., n −1.
Let us note that
...
+ 1
Rλ(n −1, i)π(n −1, i) [S(n, i + 1) −f(n −1, i)] .
(11.32)
Here and below we do not give as much detail as in earlier sections. We
challenge the reader to fill in the details.
Define
∆c(n −1, i) = RC(f(n −1, i), n) −
n−1
...
Once S(n, j) have been constructed so that (11.30) holds, we can find
...
K
Call price-ask Implied volatility-ask
4.00
0.78
na
4.25
...
K
Put price-ask Implied volatility-ask
4.00
0.02
32.07
4.25
...
C(f(n −1, i), n) −P(f(n −1, i), n) = S(0, 0) −R−nf(n −1, i)
(11.41)
...
An expiry date T. We will write T = N · ∆t, where N will be specified
below.
...
present values will be denoted by ˜Ci and will correspond to strike prices Ki for
...
these market values: Ci = ˜Ci. Rubinstein relaxes this to the requirement
that Cb
i ≤Ci ≤Ca
i , where Ca and Cb are (market) bid and ask prices for
calls. He then also relaxes S(0, 0) = S to Sb ≤S(0, 0) ≤Sb. While these
features can be included we shall require that S(0, 0) = S and Ci = ˜Ci.
...
define q(n, j) as the risk-neutral probability of a path ending at (n, j) at time
t = n. We have the recurrence
q(n + 1, j + 1) = q(n, j) · π(n, j)
(12.7)
q(n + 1, j) = q(n, j) · [1 −π(n, j)]
(12.8)
q(n + 1, 0) = q(n, 0) · [1 −π(n, 0)].
(12.9)
...
Recall that in our recombining binomial tree there are Cn
j paths leading (0, 0)
...
(12.14)
and
S(n, j) = π(n, j)S(n + 1, j + 1) + (1 −π(n, j))S(n + 1, j)
R
.
(12.15)
...
q(2, 2) = 0.6667.
Further
S(2, 0) = π(2, 0)S(3, 1) + (1 −π(2, 0))S(3, 0)
R
= 85.4184
S(2, 1) = π(2, 1)S(3, 2) + (1 −π(2, 1))S(3, 1)
R
= 98.2598
S(2, 2) = π(2, 2)S(3, 3) + (1 −π(2, 2))S(3, 2)
R
= 120.2330.
...
q(1, 1) = 0.5625
and
S(1, 0) = π(1, 0)S(2, 1) + (1 −π(1, 0))S(2, 0)
R
= 90.9987
S(1, 1) = π(1, 1)S(2, 2) + (1 −π(1, 1))S(2, 1)
R
= 109.6076
...
π(0, 0) = q(1, 1)
q(0, 0) = 0.5333
S(0, 0) = π(0, 0)S(1, 1) + (1 −π(0, 0))S(1, 0)
R
= 100.0000
...
Rn
then
Z(n, j) = π(n, j)Z(n + 1, j + 1) + (1 −π(n, j))Z(n + 1, j).
(12.19)
Therefore,
J(n) =
n
...
j=0
Cn
j · q(n, j) [π(n, j)Z(n + 1, j + 1) + (1 −π(n, j))Z(n + 1, j)]
=
n
	
j=0
Cn
j [q(n + 1, j + 1)Z(n + 1, j + 1) + q(n + 1, j)Z(n + 1, j)]
= q(n + 1, n + 1)Z(n + 1, n + 1)
+
n−1
...
j=0
Cn
j q(n + 1, j + 1)Z(n + 1, j + 1)
+
n
	
j=1
Cn
j q(n + 1, j)Z(n + 1, j) + q(n + 1, 0)Z(n + 1, 0)
= q(n + 1, n + 1)Z(n + 1, n + 1)
+
n
...
j−1

q(n + 1, j)Z(n + 1, j) + q(n + 1, 0)Z(n + 1, 0)
...
Thus
J(n) = J(n + 1)
for any n, and so the theorem has been proved.
2
...
option priced by the constructed tree with strike price Ki. As this satisfies
...
= ˜Ci,
(12.22)
so by Example 12.5 (with n = 0), Ci(0, 0) = ˜Ci.
...
Construct a CRR tree with S(0, 0) = S and N ≥m sufficiently large. Then
set
S(N, j) = S(0, 0)ujdN−j
and let
Q′
...
and change ∆t to
N
N−1 · ∆t so R is replaced by exp(r
T
N−1). We may need to
...
offour answers to four decimal places. In this example m = 4, as we had 4
...
## Complete Markets & Black-Scholes
...
In general a butterfly spread is obtained by buying calls with strikes X1 and
X3 > X1, and selling two calls with strike X2 = (X1 + X3)/2.
We note that Qj > 0 for j = 2, 3, . . . , m −1 because ˜Cj−1 −2 ˜Cj + ˜Cj+1 is
then the price of a butterfly spread.
Also Qm+1 > 0 as ˜Cm < ˜Cm−1, for we have assumed that K1 <K2 <. . . <Km.
Furthermore,
...
∆PV (K2 −K1) = 0.
Therefore, Q0 > 0
One can show that if K2 > K1 then C(K1)−C(K2) < PV (K2 −K1). In fact,
suppose the reverse holds, viz., C(K1) −C(K2) ≥PV (K2 −K1). Then short
the K1 call, buy the K2 call and invest PV (K2 −K1). At expiry date of the
...
˜Cj[Kj+1 −2Kj + Kj−1]
+ ρ
∆[ ˜C1K2 + ˜C2K3 + ˜CmKm−1 + ˜Cm−1Km−2 −2 ˜C2K2
−2 ˜Cm−1Km−1] + ρ ˜Cm
+ ρ
∆

...
∆

−2 ˜Cm−1[Km−1 −Ki] + ˜Cm−1[Km−2 −Ki] + ˜Cm[Km−1 −Ki]

+ ρ ˜Cm + ρ
∆[ ˜Cm−1 −˜Cm][Km −Ki]
=ρ ˜Ci + ρ
∆
˜Cm−1[−2Km−1 + Km−2 + Km] + ρ
∆
˜Cm[Km−1 + ∆−Km]
=ρ ˜Ci,
as required.
...
1998 algorithm. We then calculate q(10, j) =
Q(10,j)
C10
j
and we are ready to
start the One, Two, Three algorithm.
CRR −S(10, j) = 100ujd10−j where u = exp(σ
√
∆t) and d = 1/u. We note
...
Prn (S(T) ∈[K1 −0.5∆, K1 + 0.5∆]) ≈1 −1
2 [N(d2(K)) + N(d2(K + ∆))]
Prn (S(T) ∈[K12 −0.5∆, K1 + 0.5∆]) ≈1
2 [N(d2(K)) + N(d2(K −∆))]
We note that BS −Prob(10, j) ≈Q(10, j) for each j. The resulting tree in
...
12.7 Jackwerth’s Extension
169
tree price some other options (American style options, options with different
maturities, path dependent options, etc). Jackwerth [39] showed how to do
this with his generalized binomial trees.
Let us first review some earlier calculations. By (12.10) and (12.13)
Q(n, j) = Cn
j q(n, j)
= Cn
j [q(n + 1, j) + q(n + 1, j + 1)]
...

Q(n+1, j) (12.35)
where W is any function that satisfies
W(0) = 0
(12.36)
W(1) = 1
(12.37)
...
example). With a choice of weight function W, equation (12.35) defines the
backward recurrence for Q(n, j), where Q(N, j) are still defined as before. We
then define
π(n, j) ≡W
 j + 1
n + 1
...
170
12 Implied Binomial Trees
This generalizes the case where W(θ) ≡θ and Q(n, j) = Cn
j q(n, j) above.
Given S(N, j), this allows us to determine S(n, j) for each 0 ≤n < N and
j = 0, 1, 2, ..., n −1.
In fact Theorem 12.3 will still hold using (12.35) and (12.40) as defining
Q(n, j) and π(n, j).
The tree constructed by this modification is still consistent with (12.4)–(12.6).
...
five European call prices for options expiring at T = 1: C(90) = $18.14,
...
K.]
Exercise 12.15. Show that Theorem 12.3 still holds when π(n, j) is defined
via (12.40) for any choice of W that satisfies (12.36)–(12.38).
...
The difficulty with interest rate modelling is the fact that there are many
different interest rates. In Section 3.4 we introduced P(t, T), the value at time
t of 1 CAD (or one unit of domestic currency, say) at time T. Let us say this
in another way: It is the value at time t of a (fixed income) security that pays
1 CAD at time T (under all conditions, or states of the world). The value of
the fixed income security is known at expiry (T), but its values are uncertain
...
Interest rate modelling can be specified by modelling various variables.
1. P(t, T).
2. y(t, T) the yield to maturity defined through
P(t, T) = exp (−y(t, T) · (T −t))
y(t, T) = −
1
T −t ln P(t, T).
3. f(t, T) the forward rate defined in continuous time through
P(t, T) = exp

−
 T
...
One of the first inputs that we need from the market are the values of P(0, T),
...
value $100 and are classified as discount securities. If we have a 90-day bill
...
C P(0, Ti) + F P(0, Tn)
(13.2)
where F is the face value of the bond, and C is a given percentage (as specified
...
ϵ1 = 99.07 −106m(1)
ϵ2 = 100 −7m(1) + 107m(2)
ϵ3 = 105 −9m(1) + 9m(2) + 9m(3) + 109m(4).
We might even even impose an additional constraint on the parameters so that
m′(t) ≤0 for 0 ≤t ≤4 (b0 ≤0 and b0 + 8c0 ≤0). These type of curve-fitting
...
consider fitting cubic splines in much the same way as we have just described.
This means taking
m(t) = a0 + b0t + c0t2 + d0t3 +
k
...
+
(13.7)
for “knots” t1, t2, . . . , tk. We now need to find (k+3) unknowns (recall a0 = 1)
to fit the bond data exactly or with a weighted least squares method. Usually
...
$1 at time t will yield 1 + rtδ at time t + δ. Now as
(1 + rtδ)P(t, t + δ) = 1
(13.8)
we have
rt ≡rt,t+δ = 1
...
a fixed rate is paid and a floating/variable rate is received. A receiver-swap
has the reverse cash flow. If the face value (or principle) is F and the fixed
rate is κ, then the net flow is
F · rt · δ −F · κ · δ.
(13.10)
This can also be written as
F · (rt −κ) · δ = F · (1 + rtδ) −F · (1 + κδ)
= F ·
1
P(t, t + δ) −F · (1 + κδ).
This has present value (at time 0) given by
PV = F · P(0, t) −F · (1 + κδ) · P(0, t + δ)
= F · [P(0, t) −P(0, t + δ)] −F · κ · δ · P(0, t + δ).
(13.11)
Now consider a (payer-) swap where interest is exchanged at times δ, 2δ . . . , nδ.
The present value of all these cash flows together is (using (13.11) repeatedly):
PV = F · [P(0, 0) −P(0, δ)] −F · κ · δ · P(0, δ)
...
150 and 180 days. Then P(0, t) for t =
1
12 =
30
360, 2
...
1Q 5.000
1S
5.031 0.9515 0.97423 0.97605 0.0245 -0.95029
0.9270
2Q 5.125
2S
5.158 0.9031 0.97045 0.92772 0.0246 -0.89758
0.8764
3Q 5.415
3S
5.452 0.8764 0.95575 0.90086 0.0245 -0.84626
0.8376
4S 5.580
4S
5.580 0.8006 0.97018 0.82329 0.0227 -0.79698
0.7767
5S 5.680
5S
5.680 0.7535
...
are known, then we can compute (estimate) P(0, n−1
2) and P(0, n) as follows.
Regard y as a forward rate applying over [n −1, n] and suppose z is defined
...
could be obtained by assuming that the longer term rates are “flat” and we
...
P(0, T2) = P(0, T1)e−¯r(T2−T1).
(13.26)

13.2 P(0, T) from Bank Data
181
This defines a “forward” rate ¯r for the period [T1, T2]. We then make the
approximation
P(0, T) = P(0, T1)e−¯r(T −T1)
(13.27)
and so
P(0, T) = P(0, T1)
P(0, T2)
...
cash flows that fall on dates which are not integer multiples of 0.5 years ahead.
Other Discount Curves
We can now compute ˜P(0, t) for less creditworthy entities. We then have
˜P(0, t) < P(0, t) for each t, but of course ˜P(0, 0) = P(0, 0) = 1. If we are
considering the AA curve, then what we will say still applies, but we will see
that ˜P(0, t) > P(0, t) if P(0, t) stands for AA values. In fact we will consider
this last situation using hypothetical government (or semigovernment) bond
data. We first use market data to compute the discount functions from the
AA (say) data as explained above.
Since the credit rating of the government is greater than AA we will expect
˜P(0, t) > P(0, t), as we have already noted. Suppose we have data for two
...
be simplified (using properties of geometric series) to
MS1 = c1
y1

1 −
...
rate c2 = 0.075 or 7.5% again with semi-annual payments. The first coupon
after 15 August 2002 will be on 15 October 2002 and every six months after
that until expiry. Let τ be the time to the first coupon payment after 15
...
(13.30) agree. In any case the given data gives MS2 = $1.1084. We shall now
assume that
˜P(0, t) =

P(0, t) −x1t
if 0 ≤t ≤t1
P(0, t) −x1t1 −x2(t −t2) if t1 ≤t ≤t2.
(13.31)
where t1 = 0.5m1, the time to expiry of the first bond, and t2 = τ +0.5(m2−1)
...
+ ˜P(0, τ + 0.5(m2 −1))

+ ˜P(0, τ + 0.5(m2 −1)).(13.33)
The answers are x1 = −0.00095 and x2 = 0.00011. This leads us to ˜P(0.t) >
P(0, t) as we expected.
Once we have then computed the values of ˜P(0, t) we can calculate the ad-
justed swap rates:
˜κ((n + 0.5)S) = 2
⎡
⎢⎢⎢⎣
...
13.2 P(0, T) from Bank Data
183
for n = 0, 0.5, 1, 1.5, . . . using ˜P(0, 0) = 1. This recurrence is again easy to
implement, and the spread κ(nS) −˜κ(nS) can be calculated. In this example
...
As we said, one can always study specific interest rate models and decide
...
1 The authors would like to acknowledge Bob Arnold of SAFA (South Australian
...
to find a solution for all the equations in (13.37). We could then select θ so
that
∂J(θ)
∂θi
= 0,
...
A = γ + δA
1 = A + B.
Show that A = π and B = 1 −π to complete the proof.
2
Remark 13.4. The Ho and Lee model is a two-parameter model and is com-
pletely specified by values for P(0, n) for various n, and the values of π and
δ < 1.
...
only if P n
j (1) ≤1. This holds if P(n + 1)h(n) ≤P(n) for all n. This can be
...
˜Ci −Ci(π, δ)
2
where ˜Ci for i = 1, 2, . . . , M are market prices of M interest rate derivatives
...
13 Interest Rate Models
Lemma 13.7.
λ(n, j) = P(n)πj(1 −π)n−jψ(j, n −j)
$n
k=1 [π + (1 −π)δn−k]
(13.54)
where ψ(k, m) is calculated from
ψ(k, m) = δmψ(k −1, m) + δm−1ψ(k, m −1)
(13.55)
ψ(0, m) = δ
m(m−1)
2
...
λ(n, n) = πnP(n)h(0)h(1) . . . h(n −1)
λ(n, 0) = (1 −π)nP(n)h(0)h(1) . . . h(n −1)δ
n(n−1)
2
.
2
Example 13.8. Let V (N, j) = max

P N
j (T) −K, 0

...
= F(0, 0)Φ [π, δ; N, N + T] .
Exercise 13.10.
1. Find Φ and show that it is an increasing function of δ.
...
Remark 13.11. In the Ho and Lee model, π(n) ≡π and c(n) ≡1
δ are indepen-
dent of n.
Using
P n
...
g(n, T + n −1)
(13.69)
h∗(n, T) = c(n + 1)c(n + 2) . . . c(T + n −1)g(n, T + n −1).
(13.70)
Remark 13.14.
(a) We note that P n
i (T) does not depend on {π(k) ; κ ≥n}.
(b) The structure of this model is similar to the Ho and Lee model, but there
are many more parameters {c(n)} and {π(n)}.
(c) We could now proceed to find state (Arrow-Debreu) prices as in Section
...
P(0, n + 1) > 1
(13.71)
and define for j = 0, 1, 2, . . . , n
R(n, j) = R(n) un−2j .
(13.72)
We note that
R(n, j + 1) = u−2R(n, j) < R(n, j).
...
r(n, j) = r(n, 0)σ(n)j,
(13.81)
where σ(n) for n = 1, 2, . . ., are specified constants (usually greater than
...
close enough together (or the f values are sufficiently small). This method is
...
f ′(xn).
(13.87)
Very few iterations are required to find a good approximate solution.
(c) The Secant Method.
...
AVol(n, j) =
&
q (r(n + 1, j + 1) −r)2 + (1 −q) (r(n + 1, j) −r)2
(13.89)
where
r = q · r(n + 1, j + 1) + (1 −q) · r(n + 1, j).
(13.90)
Therefore,
AVol(n, j) =

q(1 −q) · |r(n + 1, j + 1) −r(n + 1, j)|
(13.91)
or, if q = 1
2,
AVol(n, j) = 1
...
In a similar way we can define proportional volatility as the standard
...
PVol(n, j) =
&
q (ln r(n + 1, j + 1) −ρ)2 + (1 −q) (ln r(n + 1, j) −ρ)2
(13.93)
where
ρ = q · ln r(n + 1, j + 1) + (1 −q) · ln r(n + 1, j)
(13.94)
and so
PVol(n, j) =

q(1 −q)| ln r(n + 1, j + 1) −ln r(n + 1, j)|
=

q(1 −q) · | ln σ(n + 1)|,
(13.95)
...
state.
When q = 1
2, we have a nice interpretation of the ln σ(n + 1) as twice the
...
q = 1
2. They assume then that q(n) has the same value for all n as in a BDT
model. The reader is referred to their paper for extensive discussions.
Let us finally note an approximate relationship between two volatilities. From
(13.92)
AVol(n, j) = 1
2 · r(n + 1, j) · |σ(n + 1) −1|
= 1
2 · r(n + 1, j) · (exp(2 · σr(n)) −1)
≈1
2 · r(n + 1, j) · (2 · σr(n))
...
about 17%, by (13.97). It should be clear that the BDT model fits well with
the market understanding of volatility.
We shall also be interested in yield volatilities.
Yields Y (0, T) and Y (n, j, T) are defined by
P(0, T) = [1 + Y (0, T)]−T
(13.98)
and
P (n)
...
(k) and P (1)
0
(k) using the values of P(0, k + 1) and v(k). As above
P(0, k + 1) =
0.5
1 + r(0, 0)

...
x.
(13.118)
It can be shown that ∆n is never 0 when xn ̸= 0 and yn ̸= 0.
...
with
Θn
i,j ≡λ0(n, j)λ1(n, i) −λ0(n, i)λ1(n, j) > 0
(13.121)
...
present the details differently.
...
0
(2) = 81.5212593.
This procedure is quite general for finding u(n) and v(n) and then the values
of P (1)
1
(n) and P (1)
0
(n) with no additional complications.
...
P(0, 3) = 71.1772, and P (1)
1
(2) = 75.0692 and P (1)
0
(2) = 81.52078. Therefore,
...
V (0, 0) = S(0, 0) + B(0, 0).
(13.123)
The debt becomes due at t = N. This means that B must be repaid by the firm
at t = N, with no repayments before this date. This is fine if V (N, j) ≥B, but
the debt-holders will only receive V (N, j) if V (N, j) < B. The debt-holders
therefore receive
˜B(N, j) = min [V (N, j), B]
(13.124)
at t = N. The present value of ˜B(N, ·) is
B(0, 0) = B ˜P(0, N).
(13.125)
It is not hard to show that ˜P(0, N) < P(0, N) whenever there is a possibility
...
compound options.
Given a binomial tree with values {V (n, j), π(n, j)} we can now find the value
...
down and solve a recurrence relation for h∗(T )
h(T ) .
...
raclius model, and find expressions for European call prices, European put
...
that π(n) has the form in (13.73) start by finding π(1) and π(2). Formula
...
j (u)
where Qn
j (u) is a polynomial in u of degree less than or equal to n2 and with
coefficients 0 or 1. Can you find Qn
j (u) explicitly?
Exercise 13.38. This is a difficult exercise. Use the Jamshidian induction
formula to show that ∆n ̸= 0 when xn and yn are nonzero. [As a hint, study
Remark 13.16. The key equation there (13.121) can be proved by mathemat-
ical induction together with (13.108) and (13.109).]
Exercise 13.39. Show that the final claim in Remark 13.16 holds.
...
208
13 Interest Rate Models
2. With this discount curve, is the following transaction profitable as of now
(19 August 2002). You borrow (receive) $100,000 on 19 October 2003 and
repay $110,000 on 19 June 2005.
3. With the values for P(0, 1), P(0, 3) and P(0, 5) from 1. find the cubic
spline to fit these values to estimate m(t) = P(0, t) for t ≤5, and compare
...
to capital investment decisions by firms. A standard book on this subject is
...
It is now widely recognized that traditional discounted cash flow
...
ture management’s flexibility to adapt and revise later decisions in
...
to invest in a project (such as a research and development effort to discover a
new drug). One year later the project will generate expected cash flows of $180
...
each outcome. The project’s cash flows have an expected rate of return (or
...
,
giving π = 0.4.
Suppose the government of the day, wishing to support the project, offers a
...
million of dollars. This traditional valuation assumes that the payoffof the
...
since the flexibility to abandon the project for a guaranteed price would alter
...
a project with present value of future cash flows V (0) (for which NPV =
...
V (1) > I(1) and not proceed otherwise. Thus the profit at t = 1 will be E(1) =
(V (1) −I(1))+, the payoffis like a call option with strike price I(1). Here,
V (1) represents the value at t = 1 of the future cash flows of the project. The
...
this added flexibility may make a nonviable project into a viable one.
Example 14.5 (Option to contract).
If market conditions become weaker, a
...
.
(14.6)
Again E(0) −I(0) > V (0) −PV0(I(1)) −I(0), so the added flexibility adds
value.
...
14.1 Examples
213
Thus C(1) = 0.30V (1). So V (1) = C(1) plus the the t = 1 value of cash flows
after t = 1. Each year management pays fixed costs (FC) plus variable costs
...
V (1) −V C −FC, and if C(1) ≤V C, then E(1) = V (1) −C(1) −FC. In
summary
E(1) = max [V (1) −V C −FC, V (1) −C(1) −FC]
= V (1) −FC −min [V C, C(1)] .
(14.7)
Therefore, the net present value of the project is E(0) −I(0). As E(1) >
V (1) −V C −FC with this option, we have a greater net present value.
...
asset then we could price these real options like any other financial option,
...
known, and for which the values S(1, ↑) ̸= S(1, ↓) are known. Here (1, ↑) and
...
sciences or indifference pricing to construct prices of real options.
14.2 Options on Non-Tradeable Assets
We have assumed in our derivative pricing that the payoffcan be replicated in
...
priced using the concept of certainty equivalence or using indifference pricing.
The concept of certainty equivalence is well known in the actuarial sciences.
We shall employ indifference pricing in this section.
...
14.2 Options on Non-Tradeable Assets
215
We shall determine this choice, called H∗
1, and V (x) the resulting maximum
expected utility.
...
(14.12)
Note that H∗
1 does not depend on initial wealth x. The number π is, as usual,
π = RS(0) −S(1, ↓)
S(1, ↑) −S(1, ↓).
...
H1 E[U(X(1) −G(1))],
(14.14)
where we note that V (x) = V0(x). Actuarial science defines the indifference
price for G to be that value ν so that
V (x) = VG(x + ν).
(14.15)
What this means is this: we are indifferent to the two situations, (1) we have
nothing to do with G, and (2) for a liability of G(1) at time t = 1, we are
happy to have extra wealth ν at t = 0. Hence the term “indifference” price.
Strictly speaking we have defined the indifference asking price, νa, rather
than the indifference bid price, νb, which could be defined by setting V (x) =
...
We can calculate the optimal H∗∗
1
in (14.14). It is calculated like the H∗
1
above. In fact, now
H∗∗
1
...
VG(x) = −exp(−γxR)E[exp(−γH∗∗
1 (S(1) −RS(0)) + γG(1)]
= −exp(−γx)(p1 exp(γg1) + p2 exp(γg2))π
ππ
...
≡π(G),
(14.20)
the risk-neutral price, as we already know. In particular S(0) = Eq

S(1)
R

...
1 + ∂νa
∂S0
where the dependence of νa on S(0) is through π. (See the definition of π.) If
we define
L(1) = νa + ∂νa
∂S0
(S(1) −RS(0))
(14.25)
then
Ep[U(L(1) −G(1))] = Ep[U(0)] = U(0),
(14.26)
so we are indifferent between L(1) −G(1) and 0.
...
Ep[U(νa −G(1))] = U(0)
Ep[U(G(1) −νb)] = U(0).
Also L(1, ↑) = L(1, ↓) = νa in this case, as
∂νa
∂S(0) = 0.
Part (b).
...
of
∂νa
∂S(0) are 0.05421, 0.03376, 0.02317, respectively. Note the dependence on
risk aversion. In the case γ = 1, L(1, ↑) = 7.75644 and L(1, ↓) = 7.26853, and
(14.26) can be verified easily. Similarly for γ = 2, 3.
Part (c).
...
values of
∂νa
∂S(0) are 0.02010, 0.01206, 0.00823. Note the dependence on risk
aversion. In the case γ = 1, L(1, ↑) = 7.75644 and L(1, ↓) = 7.57556, and
(14.25) can be verified easily. Similarly this is the case for γ = 2, 3.
Part (d).
Consider another change of the p probabilities, leaving all else unchanged.
...
be verified easily. Similarly this is the case for γ = 2, 3.
...
Ep [G | S(1) = S(1, ↑)] = p1G1 + p2G2
p1 + p2
(14.32)
Ep [G | S(1) = S(1, ↓)] = p3G3 + p4G4
p3 + p4
(14.33)
varp [G | S(1) = S(1, ↑)] =
p1p2
...
(p3 + p4)2 [G3 −G4]2 .
(14.35)
9. If H+ and H−are the payoffs of attainable claims with
H−≤G ≤H+
...
financial instruments (see [30], Chapter 5). This indicates that indifference
...
One of the problems with this indifference pricing methodology is the need
to specify the utility function u, on which the indifference price depends. If
...
νb are not market prices, but indifference prices considered from the point of
view of an agent with utility function u.
Example 14.10. As an application of these ideas, let Y (0) represent the present
value of a firm with value Y (1) at t = 1. Y (0) could represent the bid indif-
ference price of the (not publicly listed) firm from the point of view of the
CEO of the firm: Y (0) = νb(Y (1)). This same CEO may wish the value of
the firm if it he/she an embedded option to liquidate for M the firm at t = 1.
...
contracts are derivative contracts (exchange pool prices for fixed prices over
...
firm, the value of some major project and so on. Clearly, V will depend on
time. We also allow in particular V to represent the value that the CEO
assigns to the firm or project. It is not necessarily a market price, but no
doubt (and we will not be tempted to digress here), this value will also be
affected by changes in the financial markets (for example, the mining stock
sector index).
...
γR ln

p(n, j) exp(−γV (n + 1, j + 1))
+ (1 −p(n, j)) exp(−γV (n + 1, j))

,
(14.42)
...
of the firm/project at t = n + 1 together with these handouts. If we have a
...
called). This can even be simplified further if we take
V (n, j) = V (0, 0)ujdn−j
(14.45)
as in the Cox-Ross-Rubinstein (CRR) model, where u = exp(σ
√
∆t) and
d = 1/u. In that case
...
option to abandon at 900 is ˜A(0, 0) = A(0, 0) −V (0, 0) = 2.155. We give the
intermediate values in the table.
(b) The indifference pricing (IP) approach.
...
γ(n, j) · R ln

p e−γ(n,j)A(n+1,j+1) + (1 −p) e−γ(n,j)A(n+1,j)
A(n, j) = max [Z(n, j), X] ,
where the γ(n, j) values are as in the example above. We now obtain A(0, 0) =
1005.066 and ˜A(0, 0) = A(0, 0) −V (0, 0) = 5.066.
The CEO ascribes higher value to the embedded abandonment option in this
second approach. The MAD approach is linear in payoffs, and Copeland and
...
(a) the MAD approach.
We find Sh(n, j), n = 0, 1, . . . , N; j = 0, 1, . . . , n, with N = 2, L = 450,
α = 0.50, so that
Sh(N, j) = max [V (N, j), αV (N, j) + L]
Z(n, j) = 1
R [qSh(n + 1, j + 1) + (1 −q)Sh(n + 1, j)]
Sh(n, j) = max [Z(n, j), αV (n, j) + L] ,
which yields Sh(0, 0) = 1001.08, and so the value of the embedded option to
abandon at 900 is ˜
Sh(0, 0) = Sh(0, 0) −V (0, 0) = 1.08. We give the interme-
diate values in the table.

14.2 Options on Non-Tradeable Assets
225
(b) the indifference pricing approach.
We now use the recursion
Sh(N, j) = max [V (N, j), αV (N, j) + L]
Z(n, j) = −
1
...
(a) the MAD approach.
We find E(n, j), n = 0, 1, . . . , N; j = 0, 1, . . . , n, with N = 2, M = 100,
β = 0.10 so that
E(N, j) = max [V (N, j), (1 + β)V (N, j) −M]
Z(n, j) = 1
R [qE(n + 1, j + 1) + (1 −q)E(n + 1, j)]
E(n, j) = max [Z(n, j), (1 + β)V (n, j) −M] ,
which yields E(0, 0) = 1004.32, and so the value of the embedded expansion
option is ˜E(0, 0) = E(0, 0) −V (0, 0) = 4.32. We give the intermediate values
in the table.
(b) the indifference pricing approach.
We now use the recursion
E(N, j) = max [V (N, j), (1 + β)V (N, j) −M]
...
γ(n, j) · R ln

pe−γ(n,j)E(n+1,j+1) + (1 −p)e−γ(n,j)E(n+1,j)
E(n, j) = max [Z(n, j), (1 + β)V (n, j) −M] ,
where the γ(n, j) values are again as in the example above. We now obtain
E(0, 0) = 1002.06 and ˜E(0, 0) = E(0, 0) −V (0, 0) = 2.606.
...
options, the expansion option is riskier (has a more volatile payoff) and this
...
γ(n, j) · R ln

pe−γ(n,j)C(n+1,j+1) + (1 −p)e−γ(n,j)C(n+1,j)
C(n, j) = max [Z(n, j), X, αV (n, j) + L, (1 + β)V (n, j) −M] ,
where the γ(n, j) values are again as in the example above. We now obtain
C(0, 0) = 1007.31 and ˜C(0, 0) = C(0, 0) −V (0, 0) = 7.316.
...
easily calculated. (b) Because the approaches give different valuations, they
no doubt (but not in the examples provided here) lead to different decisions
...
p1, p2, p3, p4, respectively. We have three assets: cash, a financial asset (S)
and a non-tradeable asset (Y ). We have S(1, 1) = S(1, 2) = d·S and S(1, 3) =
S(1, 4) = u · S with S(0, 0) = S. We set q = (R−d)
(u−d) . We will use exponential
...
Yu = −1
γ ln
p3 exp(−γY (1, 3)) + p4 exp(−γY (1, 4))
p3 + p4

(14.51)
we can think of Yu and Yd as relative certainty equivalences and Y →˜Y =
(Yu, Yd) maps Y in some sense onto the attainable claims in the financial
market sense; and νb(Y ) = π( ˜Y ). Y can be regarded as a tradeable asset
when Y (1, 1) = Y (1, 2), and Y (1, 3) = Y (1, 4), for then it can be repli-
cated exactly by a portfolio of tradeable financial assets (cash and S). For
our model we will assume that S, Y are completely specified (we know val-
...
Y is V the value process of a firm or project. We could calibrate this model
...
then seek νb(G), the indifference bid price of G. In fact,
νb(G) = 1
R [qGu + (1 −q)Gd]
(14.53)
Gd = −1
γ ln
p1 exp(−γG(1, 1)) + p2 exp(−γG(1, 2))
p1 + p2

(14.54)
Gu = −1
γ ln
p3 exp(−γG(1, 3)) + p4 exp(−γG(1, 4))
p3 + p4

.
(14.55)
...
= G(1)
d
and G(2) can be replicated in the financial markets. Then
G(1) is valued as the actuaries do it (discount certainty equivalents) and G(2)
is valued according to financial principles (using risk-neutral expectations):
ν(G) = 1
RG(1)
u
+ π(G(2)).
...
satisfy these requirements with G(1)
u
= G(1)
d
= Gu. Equation (14.57) then
...
Let us assume that u > d (without loss of generality), and define
rS(1, j) = S(1, j) −S(0, 0)
S(0, 0)
rY (1, j) = Y (1, j) −Y (0, 0)
Y (0, 0)
...
B = pupd(Y (1, 4) + Y (1, 3) −Y (1, 2) −Y (1, 1)),

14.3 Correlation with Tradeable Assets
231
and we assume A > 0. Without going into detail, it is clear that S and Y
cannot be specified arbitrarily without introducing arbitrage opportunities
...
Example 14.20. We make a minor modification to the Copeland and Antikarov
...
p4 = 0.5411.
Using the MAD methodology of Copeland and Antikarov ignores the financial
...
232
14 Real Options
different risk aversions are implied by different views on correlations. These
...
lar constructions with (S(0, 0), Y (0, 0) replaced by, for example, (ueS(0, 0),
νfY (0, 0)), with u = exp(σS
√
∆t), ν = exp(σY
√
∆t) and e, f ∈{−1, 1}. The
probabilities will be the same as above, but the new risk-aversion parameter
will be ν−fγ for each choice of f. We can now construct the model for (S, Y )
...
occur at t = 2 when Y has its smallest value. For our approach we get different
...
with X = 1105, different decisions result. Copeland and Antikarov would give
...
This suggests that the different methodologies can lead to different optimal
decisions by our CEO.
Remark 14.22. As remarked earlier a study of this example with power utilities
will result in different pricing depending on the level of assets of the CEO, and
consequent different decisions. The Copeland and Antikarov approach does
not result in different valuations and decisions when risk aversions change,
correlations with financial markets change and when level of wealth of the
company in which a project is considered changes. The indifference approach
can capture these different aspects.
...
234
14 Real Options
The proofs of the first approximation are given in [27, equation (34)]. The
...
.
(14.59)
Let us note that when u(x) = −exp(−γx), then Γ = γ and when u(x) = 1
αxα
then Γ = 1−α
xR . When we ignore the financial asset (and we only have two
states at t = 1) then the formula further simplifies to (as now p1 + p2 = 1):
νb(G) ≈1
R

p1G1 + p2G2 −1
...
scales as Γ →Γ
β . This is quite useful in the multiperiod models as indicated
...
discounted (real world) expected payoff, then the CEO is assuming something
...
Remark 14.23. 1. As in Example 14.18 it can be shown that any payoffG
can be written as G = G(1) + G(2) with G(2) attainable (that is, G(2)(1, 1) =
G(2)(1, 2), and G(2)(1, 3) = G(2)(1, 4)), and G(1)
d
= G(1)
u
(notation as above),
...
G(2)(1, 4) = Gu and G(2)(1, 1) = G(2)(1, 2) = Gd, then G(1)
d
= G(1)
u
= 0,
...
ing offequipment (say) for $450. (An alternative could be to scale down
...
(A.1)
As
E[X2] = 12 × Pr [X = 1] + 02 × Pr [X = 0] = p,
the variance is given by
Var[X] = E[(X −E[X])2]
= (1 −p)2 × Pr [X = 1] + (0 −p)2 × Pr [X = 0]
...
ficients. Combinatorial probabilities are related to the number of possible
...
Consider box 1. We have n labelled balls, so box 1 can be filled in n ways.
Consider then box 2. Having filled box 1, there are now (n−1) balls remaining,
so box 2 can be filled in (n−1) ways. Therefore, box 1 and box 2 can be filled
in n(n −1) ways.
There will be (n −2) remaining balls to fill box 3 giving n(n −1)(n −2) ways
to fill the first 3 boxes. Continuing, there are n(n −1)(n −2) . . . (n −k + 1)
ways to fill the k boxes.
...
when the order of selection is not important. Write Cn
k (read: “n choose
k”), for this number.
Now the solution to the problem of filling k numbered boxes from the n
numbered balls could be obtained by the two following operations:
1. First choose any k balls from the n.
2. Then fill the k labelled boxes with the k chosen balls.
Step 1. can be done in Cn
k ways (the number we wish to find). Step 2. can be
done in k! ways. Therefore, step 1. followed by step 2. can be done in Cn
k · k!
ways. However, this must equal our first answer of
n!
(n−k)!. Therefore,
Cn
k =
...
241
Sn = X1 + X2 + X3 + . . . + Xn.
Then the value of Sn is the number of outcomes of 1 in the first n trials. It
...
should put p = 0.2 = ( 12
60. The probability of seven or more workers requiring
...
b(j; n, p) = Pr [Sn ≥a] .
(A.4)
This is an expression defining the complementary binomial distribution
...
note that for a = 5.7, Pr[Sn ≥5.7] = Pr[Sn ≥6]. Hence
Φ (5.7; n, p) = Φ (6; n, p)
because j ≥5.7 is the same as j ≥6. If a ≥0 is not an integer, the sum in
(A.4) is taken from 1 + ⌊a⌋, 1 plus the integer part of a (the floor function of
...
243
For example, if a = 5.7 then j > 5.7 iffj ≥6 = ⌊5.7⌋+ 1 = 5 + 1. If a is not
an integer
Pr [Sn > a] = Pr [Sn ≥a] = Φ (1 + ⌊a⌋; n, p) .
However, if a is an integer, then
Pr [Sn ≥a] = Pr [Sn > a] + b(a; n, p).
...
Suppose that E[Xn] = µ and Var[Xn] = σ2 for each n ≥1. If Xn ∼B(p) for
each n ≥1, then µ = p and σ =

p(1 −p), as we saw in Section A.1. Let
Sn = X1 + X2 + . . . + Xn
and
S∗
n = Sn −E[Sn]
...
in many books (statistics texts, finance texts, and so on), but are given with
...
= 0.999215376.
So Pr [S10 ≥7] ≈0.00078462, which agrees reasonably well with 0.00086435.
The choice n = 10 is a little small. Traditional practice says we need n to be
about 30 or more.
A.5 Berry-Ess´een Theorem
This theorem says something about the difference between Gn(x) and N(x)
defined above. In fact Berry (1941) and Ess´een (1945) (see Chung [15])
showed that
sup
x |Gn(x) −N(x)| ≤C E(|X1 −µ|3)
σ3√n
(A.8)

246
A The Binomial Distribution
for all n. This means that the difference magnitude |Gn(x) −N(x)| is less
than the right hand side in (A.8) for all x. In fact Berry and Ess´een gave
C = 33
4 , but a more precise argument gives C ≈0.7975, and it can be shown
than in general C ≥(2π)−1
2 .
For the case where Xn ∼B(p) for all n, µ = p and σ =

p(1 −p), and
...
∆t)
where ∆t = T
N . Using Taylor approximations (and order notation),
π = r −1
2σ2
2σ
√
...
a complete model for assets. This means that all payoffs can be hedged and
...
80 · x + 1.01 · y = 2
75 · x + 1.01 · y = 0.
It is easy to see that the unique solution of the first two equations does not
solve the third.
...
We now find x∗, y∗and C∗.
Let us note that the linear programme (B.2) is feasible. We can take x = 1
and y = 0. This choice satisfies the constraints (B.1).
...
−(S(1) −K)+ + x∗· S(1) + 1.01 · y∗≥0.
(B.4)
(recall y∗< 0). The inequality (B.4) follows because (x∗, y∗) satisfies the three
inequalities in (B.2). Therefore, there is an arbitrage opportunity. There is a
positive profit at t = 0 and no unfunded liabilities at t = 1.
...
asset return is the difference between an asset return and the risk-free interest
rate.)
B.3 The Duality Theorem of Linear Programming
...
gi(x) ≤0,
i = 1, 2, . . . , m.
Here f, gi (i = 1, 2, . . .) are sufficiently smooth functions.
...
Then if (x∗, λ∗) satisfies conditions 1.–4. of Theorem B.3, then x∗solves the
...
For this it suffices to show feasibility (there is an x ∈Rn with Ax ≥c) and
...
cT y∗= sup{cT y|
AT y = b, y ≫0}.
Proof. Clearly cT y∗≥sup{cT y|
AT y = b, y ≫0}.
Consider ϵ such that 0 < ϵ ≤1. Then yϵ = (1 −ϵ)y∗+ ϵy0 ≫0 for all such ϵ.
Then
yϵ = (1 −ϵ)cT y∗+ ϵcT y0 →cT y∗
as ϵ →0+.
...
As there is no arbitrage, there is a y ≫0 with AT y0 = b. In fact y0 = π
R for
...
K
K1
Fig. B.1. Representation of a finite state market.
...
f(ϵ) = ∥a −(m + ϵx)∥2 ,
ϵ ∈R.
Then f(ϵ) has a minimum at ϵ = 0, so f ′(0) = 0. That is
f(ϵ) = ∥a −m∥2 −2ϵ ⟨a −m, x⟩+ ϵ2 ∥x∥2 .
So f ′(0) = 0 implies ⟨p, x⟩= ⟨a −m, x⟩= 0.
2
Lemma B.10. p ≫0 (that is, all components of p are strictly positive).
Proof. Let e ∈K1, 0 ≤ϵ ≤1, and write
g(ϵ) = ∥ϵe + (1 −ϵ)a −m∥2 .
Clearly, ϵe + (1 −ϵ)a ∈K1 whenever 0 ≤ϵ ≤1. So g has a minimum on [0, 1]
when ϵ = 0, which implies g′(0) ≥0.
Now
g(ϵ) = ∥p∥2 −2ϵ ⟨e −a, p⟩+ ϵ2 ∥e −a∥2
...
This equivalence is the first fundamental theorem of finance.
...
π ≫0 and AT π = Rb. If there is no arbitrage, then P ̸= ∅.
Duality
An introduction to linear programming can be obtained from Chv´atal’s
...
K states, say), let us write that if π1 and π2 ∈P, then there is a claim C
so that C∗̸= C∗(so the market is incomplete). In fact, if π1
j < π2
j , C(1, j) =
1, C(1, i) = 0, i = j, then
C∗= inf
...
⎬
⎭.
C is attainable if and only if C1π1 +C2π2 +C3π3 is independent of t. That is,
(2C1 −3C2 + C3)t + (−1
2C1 + 3
2C2)
...
RS1(0) = p1S1(1, ↑) + p2S1(1, →) + p3S1(1, ↓)
RS2(0) = p1S2(1, ↑) + p2S2(1, →) + p3S2(1, ↓)
1 = p1 + p2 + p3.
This system has a unique solution: p1 = p2 = p3 =
1
3 and R = 1, but
...
costs.
There are 6 constraint equations here. The difficulty is how to deal with
...
is this factor that usually most effects an option price. Practitioners sometimes
...
for the underlying. Perhaps this is one of the most important tasks in finance—
...
Assume that the underlying asset price S satisfies
ln
S(tj+1)
S(tj)

...
tj+1 −tj · ϵj+1
(C.1)
where 0 = t0 < t1 < t2, . . ., and the random variables ϵj are all independent
...
estimator for σ2∆t. Equation (C.6) follows from (C.4), and may simplify the
calculation of ˆσ2.
...
ˆµ = 0.00476,
ˆσ2 = 0.000151357
or ˆσ = 0.00123. We are not so concerned about ˆµ as this does not enter option
pricing. But ˆσ is an estimator for σ2∆t, or the daily volatility. We must decide
...
volatility) of 0.00123 ×
√
252 = 0.195. The estimated volatility is 19.5% per
annum. (The standard error of this estimate is
0.195
√2×20 or 3.1% per annum.)
Remark C.2.
1. What is the daily price? Hull in the example considers the
...
stock. Modifications can be made for dividends —see Hull [ibid., page
...
amples different models to C.1 may be required to capture market features
(stylized facts).
C.2 Implied Volatility Estimation
Implied volatility is calculated in a different way. We still assume the model
...
how to go from an estimate σn to σn+1. Of course we assume f(σn) ̸= 0, else
...
a computational point of view, it would be good to program the functions
C = C(σ) ≡SN(d1) −Ke−rT N(d2) and V (σ) ≡Sσ
√
TN ′(d1) with input
...
from the above discussion that σ0 > σimp. We now need to find another guess
...
closer and closer. If we require an accuracy of ϵ for σimp, then we continue
...
ln(2) .
(C.18)
If we took σ0 = 20%, and ϵ = 0.01%, then (C.18) means k > 10.97, that is,
...
ϵ = 0.0001. Since the option’s actual volatility is not greater than 20% (check
that f(0.20) > 0), we define σ1 by
σ1 = σ0 −1
2σ0 = 0.10.
...
required in the final answer. This approach is taken in all further steps.
...
Remark C.5.
1. Of course both approaches could be applied more generally.
...
options are priced as the difference of two option prices) in the same way
...
of model (C.1).
5. If we keep S, r, T fixed and compute σimp for various values of strike K,
we can get different answers. This gives rise to the phenomenon called
...
titioners might use a spline function to fit these data points. Then, for
...
about this. It is also possible that these estimates could be quite different.
...
11. Suppose now that we keep S, K, r fixed, and calculate σimp for various
values of T. If model (C.1) is correct then again we should always get the
same answer. However, we often obtain different answers. This means that
the (annualized) volatilities for different times to expiry are not the same.
...
Then S is a closed convex subset of Rm and b ̸∈S.
...
for 0 ≤t ≤1. Then from the definition of y∗, ψ has a minimum at t = 0, and
so ψ′(0) ≥0, from which (D.6) follows.
Let w = b −y∗̸= 0. We claim that this choice satisfies (D.5).
In fact, using 0 ∈S in (D.6),
w′b = ⟨b −y∗, b⟩
= ⟨b −y∗, b −y∗⟩+ ⟨b −y∗, y∗⟩
1 x = (x1, x2, . . . , xn)′ ≥0 means xi ≥0 for all i = 1, 2, . . . , n.
2 w′ denotes the transpose of the vector w.
3 If x, y ∈Rn, say, then ⟨x, y⟩= x1y1 + x2y2 + . . . + xnyn, the scalar (dot) product
of x and y.
...
shown that w defined above satisfies (D.5). We have thus established that if
(D.5) has no solution, then (D.4) must have a solution (for if not . . . ).
2
D.2 An Application to the Problem
...
⎠
and
4 ei = (0, 0, .., 1, . . . , 0)′ with 1 in the ith spot.
...
2) × (N
+
1) matrix. Let us suppose that w =
(w0, w1, w2, . . . , wm, wm+1)′ satisfies (D.5). Then this means that (on dividing
the right inequality in (D.5) by ρ)
w0
1
ρ

+ w1 ˜C1 + w2 ˜C2 + . . . + wm ˜Cm + wm+1S > 0
and
w0 + w1(S(N, 0) −K1)+ + . . . + wm(S(N, 0) −Km)+ + wm+1S(N, 0) ≤0
w0 + w1(S(N, 1) −K1)+ + . . . + wm(S(N, 1) −Km)+ + wm+1S(N, 1) ≤0
...
w0 + w1(S(N, N) −K1)+ + . . . + wm(S(N, N) −Km)+ + wm+1S(N, N) ≤0
We now perform the trades: If w0 > 0, borrow 1
ρ from the bank, if w0 < 0,
invest 1
ρ in a bank; otherwise do nothing.
...
+ w1 ˜C1 + w2 ˜C2 + . . . + wm ˜Cm + wm+1S > 0,
and at t = N we have
−w0 −w1(S(N, ·) −K1)+ −. . . −wm(S(N, ·) −Km)+ −wm+1S(N, ·) ≥0.
...
Qj ≥0 for j = 0, 1, 2, . . . , M iffthere are no arbitrage opportunities.

E
Some Generalizations
This note gives a generalization to some material in Section 12.5 of van
der Hoek’s 1998 Method. A key component was to find Qj ≥0 : j =
...
Lemma E.1. Let f ∈C2[0, ∞) and have compact support. Then
f(x) = f(0) + xf ′(0) +
 ∞
0
f ′′(y)(x −y)+ dy
(E.4)
for all x ≥0.
Proof. This follows from simple integration by parts.
2
...
0
if x ≥K3.
the payoffof a (call) butterfly spread.
...
We assume K1 < K2 < . . . . < Km. ˜Cj is the (market) price of a European
call option expiring at time T with strike price Kj. N = m −1, as we shall
see.
ρ = exp(r · T)
or
P(0, T)−1
We first write the choices for Qj: For j = 2, 3, . . . , m −1,
Qj = ρ

1
Kj −Kj−1
...
butterfly spread, which must be positive. Qm+1 > 0 as ˜Cm−1 > ˜Cm, and
Q0 = 1 −
ρ
K2 −K1

...

> 0,
as it is ρ times the present value of and asset with pay-off
f(S(T)) =
⎧
⎨
⎩
...
K2 −K1
˜C2 > 0.
2. It is now not difficult to show that (E.2) and (E.3) hold. For (E.3) we
need to use ˜C1 ≥S −K1
ρ , which implies that K1 ≥S0. Equation (E.3) is
...
is a continuously differential piecewise cubic function, that is,
m(t) = ai + bit + cit2 + dit3,
ti ≤t ≤ti+1,
(F.1)
...
ti =
i
k+1T, that is equal-spaced, but this is not the best thing to do. If you
...
t1, t2, . . . , tk are chosen and fixed throughout the discussion.
...
g(t),
t > t1
and f, g are sufficiently smooth, then h(t1+), h′(t1+), h′′(t1+) stand for re-
...
bi−1 + 2ci−1ti + 3di−1t2
i = bi + 2citi + 3dit2
i
(F.4)
2ci−1 + 6di−1ti = 2ci + 6diti
(F.5)
for i = 1, 2, . . . , k. As (F.3)–(F.5) are awkward to deal with, Litzenberger and
Rolfo [48] proposed first to write m is a different way.
F.1 An Alternative representation of Function (F.1)
By (F.1) on [t1, t2],
m(t) = a1 + b1t + c1t2 + d1t3.
(F.6)
We wish to write it as
m(t) = a0+b0t+c0t2+d0t3+A1+B1(t−t1)+C1(t−t1)2+F1(t−t1)3 . (F.7)
...
C1 = c1 −c0 + 3F1t1
= c1 −c0 + 3(d1 −d0)t1
B1 = (b1 −b0) + 2C1t1 −3F1t2
1
= (b1 −b0) + 2(c1 −c0)t1 + 3(d1 −d0)t2
1
A1 = (a1 −a0) + B1t1 −C1t2
1 + F1t3
1
= (a1 −a0) + (b1 −b0)t1 + (c1 −c0)t2
1 + (d1 −d0)t3
1
after doing a little algebra, and this expresses A1, B1, C1, F1 in terms of
a1, b1, c1, d1. So on [t0, t1] we can write
m(t) = a0 +b0t+c0t2 +d0t3 +[A1 +B1(t−t1)+C1(t−t1)2 +F1(t−t1)3]D1(t)

F.3 Unknown Coefficients
291
...
ai−1 + Bi−1t + ci−1t2 + di−1t3 + Ai + Bi(t −ti) + Ci(t −ti)2 + Fi(t −ti)3
as an alternative to ai +bit+cit2 +dit3. Again we can express Ai, Bi, Ci, Fi
...
j=1

Aj + Bj(t −tj) + Cj(t −tj)2 + Fj(t −tj)3
Dj(t)
where Dj(t) = 0 for t < tj and Dj(t) = 1 for t ≥tj.
F.2 Imposing Smoothness
It is not difficult to show from (F.3)–(F.5) that
m(tj+) −m(tj−) = Aj
m′(tj+) −m′(tj−) = Bj
m′′(tj+) −m′′(tj−) = 2Cj
...
We will have different choices of {C, F, T1, T2, . . . , Tn, n} for different bonds.
...
F.5 Determination of Unknown Coefficients
293
η = C
n
...
i=1
Dj(Ti)(Ti −tj)3 + FDi(Tn)(Tn −tj)3
which are computable for each bond that we observe in terms of C, F, T1, T2,
. . . , Tn.
F.5 Determination of Unknown Coefficients
We determine the unknown coefficients b0, c0, d0, F1, F2, . . . , Fk by linear re-
...
2
(F.14)
where wl > 0 for l = 1, 2, ..., N and )N
l=1 wl = 1. For a least squares approxi-
mation we would normally set wl = 1/N for all l, but we have the flexibility
to give more weight to certain data. To find ˆα optimal, we solve:
∂J
∂αj
= 0,
j = 1, 2, . . . , M
...
j
(F.17)
for j = 1, 2, . . . , M. This is a matrix equation for ˆα which is
V ˆα = β
(F.18)
with β = (β1, β2, . . . , βM) and βj = )N
l=1 wlylxl
j and V = (Vij), and
Vij =
N
...
j
(F.19)
We need to show that V is invertible. In fact if V is positive definite, then
V is invertible. This will be the case if
ξT V ξ ≥0
for any ξ ∈RM
and ξT V ξ = 0 implies ξ = 0. But
ξT V ξ =
M
...
2
≥0
and ξT V ξ = 0 implies )M
i=1 ξixl
i = 0 for l = 1, 2, . . . , N which implies
)M
i=1 ξixi = 0 where xi = (x1
i , x2
i , . . . , xN
i ) ∈RN. This will imply that ξi = 0

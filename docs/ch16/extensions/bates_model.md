...
Recall Heston dynamics (see [§ Heston SDE and Parameters](../model_definition/heston_sde_and_parameters.md)) and the Merton lognormal-jump compound Poisson process (see [§ Merton Jump Diffusion](../../ch07/merton_jump_diffusion/calibration_to_smile.md)). The diffusive Heston skew vanishes as $\sqrt{T}$, so it cannot reproduce the **steep short-maturity smiles** observed in equity markets, where single jumps dominate short-dated option premia.

The **Bates model** (Bates, 1996) resolves this by combining Heston stochastic volatility with **Merton-type lognormal jumps** in the stock price. The model retains the affine structure (see [§ Affine Structure and Riccati](../model_definition/affine_structure_and_riccati.md)), so the characteristic function is closed-form and all Fourier pricing methods apply unchanged.
...
Recall the Heston CF (see [§ Closed-Form Characteristic Function](../heston_cf/closed_form_characteristic_function.md)):

$$
\phi_{\text{Heston}}(u, \tau) = \exp\!\left(iu(X_t + (r - q - \lambda\bar{k})\tau) + C(\tau, u) + D(\tau, u) v_t\right)
$$

with $C, D$ the standard Riccati solutions and drift adjusted by $-\lambda\bar{k}$.
...
Recall the Heston QE scheme (see [§ Variance Dynamics](../variance_dynamics/cir_variance_process_solution.md)) and Merton jump simulation (see [§ Merton Jump Diffusion](../../ch07/merton_jump_diffusion/calibration_to_smile.md)). The Bates discretization augments QE with jumps:

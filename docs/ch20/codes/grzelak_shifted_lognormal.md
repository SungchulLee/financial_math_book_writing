...
**Exercise 1.**
The shifted lognormal model defines $dF = \sigma(F + s)\,dW$ where $s$ is the shift parameter. Explain why this model was widely adopted after rates turned negative.

??? success "Solution to Exercise 1"
    The standard lognormal (Black) model requires $F > 0$ and has $dF = \sigma F\,dW$, which cannot handle negative rates. The shifted model replaces $F$ with $F + s$, where $s > 0$ is chosen so that $F + s > 0$ even when $F < 0$. For example, with $s = 3\%$ and a rate at $-1\%$, the effective level is $2\%$, which is positive. The lognormal dynamics then apply to $F + s$, preserving the Black formula structure (and hence all existing pricing/hedging infrastructure) while accommodating negative rates.

---

**Exercise 2.**
Derive the shifted Black formula for a caplet with forward rate $F$, shift $s$, strike $K$, and volatility $\sigma_{\text{shifted}}$.

??? success "Solution to Exercise 2"
    Substitute $\tilde{F} = F + s$ and $\tilde{K} = K + s$ into the standard Black formula:

    $$
    \text{Caplet} = \tau\,P(0,T_2)\bigl[\tilde{F}\,\mathcal{N}(d_1) - \tilde{K}\,\mathcal{N}(d_2)\bigr],
    $$

    where

    $$
    d_1 = \frac{\ln(\tilde{F}/\tilde{K}) + \frac{1}{2}\sigma_{\text{shifted}}^2 T}{\sigma_{\text{shifted}}\sqrt{T}}, \quad d_2 = d_1 - \sigma_{\text{shifted}}\sqrt{T}.
    $$

    The shift $s$ must be large enough that $\tilde{F} > 0$ and $\tilde{K} > 0$ for all relevant strikes.

---

**Exercise 3.**
If the market quotes a normal (Bachelier) volatility of 50 bps for an ATM caplet with $F = 2\%$, and the shift is $s = 3\%$, estimate the corresponding shifted lognormal volatility.

??? success "Solution to Exercise 3"
    The approximate conversion between normal vol $\sigma_N$ and shifted lognormal vol $\sigma_{\text{SLN}}$ at the money is:

    $$
    \sigma_N \approx \sigma_{\text{SLN}} \times (F + s),
    $$

    so

    $$
    \sigma_{\text{SLN}} \approx \frac{\sigma_N}{F + s} = \frac{0.0050}{0.02 + 0.03} = \frac{0.0050}{0.05} = 0.10 = 10\%.
    $$

    The shifted lognormal vol is approximately $10\%$.

---

**Exercise 4.**
Compare the implied volatility smile generated by the shifted lognormal model versus the SABR model. Which is more flexible?

??? success "Solution to Exercise 4"
    The shifted lognormal model produces a symmetric smile around the ATM point (since it is still lognormal in the shifted variable). It has only two free parameters ($\sigma_{\text{shifted}}$ and $s$), limiting its ability to capture asymmetric skews. The SABR model ($dF = \sigma F^\beta\,dW_1$, $d\sigma = \alpha\sigma\,dW_2$, $\rho$) has four parameters ($\alpha, \beta, \rho, \nu$) and can generate asymmetric skews and smiles of varying shapes. SABR is much more flexible and is the industry standard for swaption and cap volatility interpolation, while shifted lognormal is used mainly as a quick fix for negative rates.

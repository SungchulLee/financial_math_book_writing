...
Recall (see [§ Forward LIBOR Dynamics](forward_libor_dynamics.md)) the full derivation of LIBOR dynamics under the $T_{i+1}$-forward, terminal $\mathbb{Q}^{T_n}$, and spot $\mathbb{Q}^B$ measures, with Girsanov drift adjustments. Recall (see [§ LMM Drift in Terminal Measure](lmm_drift_terminal_measure.md)) the explicit terminal- and spot-measure drift formulas. The driving Brownian motions are correlated, $dW_i \cdot dW_j = \rho_{ij}\,dt$.

Key facts: $L_i$ is a martingale under its natural measure $\mathbb{Q}^{T_{i+1}}$; under $\mathbb{Q}^{T_n}$ the rate $L_{n-1}$ is a martingale and others acquire negative drifts; under $\mathbb{Q}^B$ all drifts are positive.

---

## Connection to HJM

Recall (see [§ HJM](../hjm/forward_rate_dynamics.md)) the HJM framework. The LMM arises from HJM with the specific instantaneous-forward volatility

$$
\sigma_{\text{HJM}}(t, T) = \sum_{i} \sigma_i(t) \cdot \mathbf{1}_{T_i < T \leq T_{i+1}} \cdot \frac{\delta_i L_i(t)}{1 + \delta_i L_i(t)}
$$

which yields lognormal discrete forward rates and automatically satisfies the HJM no-arbitrage drift condition.

---

## Caplet and Swaption Pricing

Recall (see [§ Caplet Pricing and Black's Formula](caplet_pricing_black_formula.md)) the Black caplet formula

$$
\text{Caplet}_i = \delta_i P(0, T_{i+1})\bigl[L_i(0) N(d_1) - K N(d_2)\bigr],
$$

with integrated variance $v_i^2 = \int_0^{T_i}\sigma_i^2\,dt$ and $\sigma_i^{\text{Black}} = \sqrt{v_i^2/T_i}$.

For swaptions, recall (see [§ Swaption Approximations](swaption_approximations.md)) that the swap rate is not lognormal in the LMM and Rebonato's formula provides the standard analytical approximation.

---

## Calibration

Recall (see [§ Calibration](../calibration/fitting_initial_yield_curves.md)) the cap/swaption calibration workflow: caps fix individual $\sigma_i$, swaptions fix the correlation $\rho_{ij}$. Recall (see [§ Correlation Parameterization](correlation_parameterization.md)) the standard parametric forms ($\rho_{ij} = e^{-\beta|T_i - T_j|}$, two-parameter, angle-based).
...
Recall (see [§ LMM Simulation and Discretization](lmm_simulation_and_discretization.md)) the log-Euler scheme, drift approximations (frozen drift, predictor--corrector), Cholesky/rank-reduced sampling of correlated normals, and the choice of simulation measure.

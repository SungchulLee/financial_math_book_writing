...
## What Is Yield Curve Construction?
...
### Instruments for Curve Construction
...
## Economic Interpretation: What the Curve Reveals
...
### The Three Theories of Term Structure
...
### Phase 2: Bootstrapping Zero Rates
...
### Phase 4: Deriving Par and Forward Curves
...
# Output: 4-year 5% coupon bond price: $101.23
```
...
### Example 1: Normal Curve - December 2024
...
### Example 2: Inverted Curve - October 2022 (Peak Inversion)
...
### Example 3: Steep Curve - March 2020 (COVID Crash)
...
### Example 4: Flat Curve - December 2018 (Fed Pause)
...
### Example 5: Swap Curve vs. Treasury Curve - Spread Analysis
...
### The Perfect Yield Curve Construction

**Setup for accuracy:**

**Ideal conditions:**

1. **Liquid market** (tight bid-ask spreads)
2. **Complete instrument set** (all maturities covered)
3. **Stable conditions** (no crisis, smooth data)
4. **Consistent methodology** (standardized approach)
5. **Validation** (arbitrage-free, smooth)

### Best Case Example: On-the-Run Treasury Curve (Normal Market)

**Background: Stable market, December 2024**

**Data collection:**

```python
# Perfect data set (all on-the-run Treasuries)
instruments = pd.DataFrame({
    'maturity': [0.0833, 0.25, 0.5, 1, 2, 3, 5, 7, 10, 20, 30],
    'instrument': ['1M Bill', '3M Bill', '6M Bill', '1Y Bill',
                   '2Y Note', '3Y Note', '5Y Note', '7Y Note',
                   '10Y Note', '20Y Bond', '30Y Bond'],
    'price': [99.56, 98.69, 97.38, 94.90, 99.45, 98.70,
              97.20, 96.45, 94.95, 89.95, 88.45],
    'coupon': [0, 0, 0, 0, 4.75, 4.50, 4.25, 4.25,
               4.00, 4.50, 4.50],
    'bid_ask_spread': [0.005, 0.005, 0.01, 0.01, 0.015,
                       0.015, 0.02, 0.02, 0.02, 0.03, 0.03]
})
```

**Bootstrapping (clean execution):**

```python
# All instruments liquid, no outliers
zeros = bootstrap_all_instruments(instruments)

# Validation: Check for smoothness
diffs = zeros['zero_rate'].diff()
print(f"Max rate change: {diffs.max():.4f}")  # 0.0012 (smooth!)
print(f"Mean rate change: {diffs.mean():.4f}")  # 0.0003 (gradual)

# Check for arbitrage
arbitrage_profits = check_arbitrage_opportunities(zeros, instruments)
print(f"Max arbitrage profit: ${arbitrage_profits.max():.2f}")  # $0.05 (tiny!)
```

**Interpolation (cubic spline, perfect fit):**

```python
cs = CubicSpline(zeros['maturity'], zeros['zero_rate'])

# Evaluate smoothness (second derivative)
t_grid = np.linspace(0.25, 30, 1000)
second_deriv = cs(t_grid, 2)
print(f"Max curvature: {np.abs(second_deriv).max():.6f}")  # 0.000012 (very smooth!)
```

**Applications (pricing validation):**

```python
# Price synthetic bonds, compare to market
test_bonds = [
    {'coupon': 4.50, 'maturity': 4.5, 'market_price': 98.75},
    {'coupon': 4.75, 'maturity': 6.5, 'market_price': 99.20},
    {'coupon': 4.25, 'maturity': 15, 'market_price': 93.50}
]

for bond in test_bonds:
    theoretical_price = price_bond_from_curve(
        bond['coupon'], bond['maturity'], cs
    )
    error = abs(theoretical_price - bond['market_price'])
    print(f"{bond['maturity']}Y: Market ${bond['market_price']:.2f}, "
          f"Model ${theoretical_price:.2f}, Error ${error:.2f}")

# Output:
# 4.5Y: Market $98.75, Model $98.76, Error $0.01
# 6.5Y: Market $99.20, Model $99.19, Error $0.01
# 15Y: Market $93.50, Model $93.51, Error $0.01
```

**Result: Nearly perfect pricing (errors <$0.02)**

**Why this worked:**

1. **Complete data:** All key maturities represented
2. **Liquid instruments:** Tight spreads, accurate prices
3. **Stable market:** No stress, clean price discovery
4. **Proper bootstrapping:** Sequential, no forced fits
5. **Appropriate interpolation:** Cubic spline for smoothness
6. **Validation:** Arbitrage checks, pricing tests
7. **Application:** Consistent pricing across all bonds

**This represents yield curve construction perfection:**

- Error: <2 cents per $100 face value
- Smoothness: No kinks or oscillations
- Arbitrage-free: No free money opportunities
- Predictive: Accurate pricing of off-market bonds

---

## Worst Case Scenario

### The Yield Curve Construction Disaster

**Worst possible conditions:**

1. **Illiquid market** (wide spreads, stale prices)
2. **Missing instruments** (gaps in maturity spectrum)
3. **Crisis conditions** (flight-to-quality, distortions)
4. **Inconsistent data** (different sources, timing)
5. **Poor methodology** (wrong interpolation, no validation)

### Worst Case Example: March 2020 COVID Crash (Treasury Market Dysfunction)

**Background: Unprecedented Treasury market stress**
...
instruments = pd.DataFrame({
    'maturity': [0.25, 1, 2, 5, 10, 30],  # GAPS! No 3Y, 7Y, 20Y
    'price': [99.88, 99.75, 102.15, 105.80, 112.50, 138.25],
    'coupon': [0, 0, 1.625, 1.500, 1.750, 2.375],
    'bid_ask_spread': [0.02, 0.05, 0.25, 0.50, 0.75, 1.50],  # WIDE!
    'last_trade': ['10 min ago', '30 min ago', '2 hours ago',
                   '4 hours ago', '1 hour ago', '3 hours ago'],  # STALE!
})
```

**Issues:**

1. **Huge gaps:** No 3-year, 7-year, 20-year (must interpolate wildly)
2. **Wide spreads:** 1.50 for 30-year (vs. 0.03 normal) = 50x wider!
3. **Stale prices:** Some bonds hadn't traded in hours
4. **Volatile:** Prices jumping 2-3% intraday

**Bootstrapping (garbage in, garbage out):**
...
try:
    ...
    zeros = bootstrap_with_errors(instruments)
...
```
  maturity  zero_rate  error
0     0.25   0.00125    NaN
1     1.00   0.00250    NaN
2     2.00  -0.00500   HIGH  # NEGATIVE RATE! (impossible)
3     5.00   0.01200   HIGH
4    10.00   0.00850   HIGH  # Lower than 5Y! (inverted long end, unusual)
5    30.00   0.01100   HIGH
```

**Disaster: Curve makes no economic sense**

- 2-year negative (shouldn't be)
- 10-year below 5-year (extreme inversion)
- Likely errors in bond prices or bootstrapping

**Attempted interpolation (compounds errors):**

```python
cs = CubicSpline(zeros['maturity'], zeros['zero_rate'])

# Check 3-year (missing data point)
z_3y = cs(3.0)
print(f"Interpolated 3-year zero: {z_3y:.4f}")  # -0.0025 (NEGATIVE!)

# Check smoothness
t_grid = np.linspace(0.25, 30, 1000)
rates = cs(t_grid)
print(f"Min rate: {rates.min():.4f}")  # -0.0180 (nonsense!)
print(f"Max rate: {rates.max():.4f}")  # 0.0250
```

**Cubic spline creates wild oscillations between sparse, bad data points**

**Pricing validation (catastrophic):**

```python
# Price a known bond: 3-year, 1.50% coupon, should be ~$100
test_price = price_bond_from_curve(1.50, 3.0, cs)
print(f"3-year 1.50% bond theoretical price: ${test_price:.2f}")

# Output: $103.75 (actual market: $99.50, error $4.25!)
```

**$4.25 pricing error on $100 bond = 4.25% off!**

**Real-world impact:**

**Trader uses bad curve:**

```python
portfolio = [
    {'coupon': 2.00, 'maturity': 4.0, 'position': 1000000},  # $1M face
    {'coupon': 1.75, 'maturity': 7.0, 'position': 2000000},  # $2M face
    {'coupon': 2.50, 'maturity': 15, 'position': 5000000}   # $5M face
]

total_error = 0
for bond in portfolio:
    model_price = price_bond_from_curve(bond['coupon'], bond['maturity'], cs)
    actual_price = get_market_price(bond)  # From broker
    error_per_100 = model_price - actual_price
    total_error += (error_per_100 / 100) * bond['position']

print(f"Total portfolio valuation error: ${total_error:,.0f}")

# Output: Total portfolio valuation error: $-285,000
```

**Portfolio marked $285,000 too low due to bad curve!**

**What went catastrophically wrong:**

**1. Data quality:**
- Stale prices (hours old in fast market)
- Wide spreads (no confidence in mid-price)
- Missing instruments (huge interpolation gaps)

**2. Market structure:**
- Treasury dealers withdrew liquidity
- Circuit breakers in stocks spilled to bonds
- Fed not yet intervening (came March 23)

**3. Methodology:**
- Blindly bootstrapped bad data
- Cubic spline with sparse points created oscillations
- No sanity checks (negative rates accepted!)

**4. Validation:**
- Skipped arbitrage checks (too urgent, crisis mode)
- Didn't cross-validate with swap curve
- Didn't wait for market to stabilize

**5. Application:**
- Used curve for mark-to-market (huge errors)
- Generated bad hedge ratios
- Created phantom P&L

**Lessons:**

1. **Garbage in, garbage out:** Bad data → bad curve → bad decisions
2. **Crisis ≠ normal:** Different methodology needed during stress
3. **Validate obsessively:** Arbitrage checks, cross-curve validation
4. **Don't force fits:** If data is bad, admit you don't have a curve
5. **Use multiple sources:** Swap curve, dealer quotes, historical relationships
6. **Wait for Fed:** In March 2020, Fed intervened March 23 (QE), market stabilized within days

**Better approach (March 2020):**

- Use previous day's curve + shift (not complete rebuild)
- Cross-validate with swap curve (more stable)
- Widen error bars (acknowledge uncertainty)
- Flag positions as "low confidence" until market normalizes
- **Most important: Don't trade or mark portfolio until data clean**
...
**Yield curve construction builds a continuous term structure from discrete market instruments using bootstrapping and interpolation:**

$$
\text{Market Prices} \xrightarrow{\text{Bootstrap}} \text{Zero Rates} \xrightarrow{\text{Interpolate}} \text{Continuous Curve}
$$

- Start with liquid instruments (bills, notes, bonds, swaps)
- Bootstrap zero rates sequentially (short to long)
- Interpolate gaps (cubic spline, Nelson-Siegel)
- Validate for arbitrage (no free money)
- Derive par and forward curves (from zero curve)
- Apply for pricing (bonds, swaps, derivatives)

### The Setup

**Standard US Treasury curve:**

- Bills (0-1 year): Direct conversion to zeros
- Notes (2-10 years): Bootstrap from coupon bonds
- Bonds (20-30 years): Bootstrap long end
- Interpolation: Cubic spline for smoothness
- Validation: Price all instruments, check errors <$0.10
...
$$
P = \sum_{i=1}^{n} \frac{C_i}{(1+z_i)^{t_i}} + \frac{100}{(1+z_n)^{t_n}}
$$

**Forward rate:**

$$
f(t_1,t_2) = \frac{(1+z_2)^{t_2}}{(1+z_1)^{t_1}}^{\frac{1}{t_2-t_1}} - 1
$$
...
- **Normal (upward):** Growth expectations, positive term premium
- **Inverted (downward):** Recession expectations (70% accuracy within 12-18 months)
- **Flat:** Uncertainty, transition, equilibrium
- **Humped:** Short-term tightening, long-term easing

**Forward rates reveal expectations:**

- Upward sloping forwards → Market expects rising rates
- Downward sloping forwards → Market expects falling rates

### Common Mistakes

1. Using stale or illiquid data (crisis periods)
2. Ignoring gaps in maturity spectrum (forced interpolation)
3. Wrong interpolation method (linear creates kinks, overfitted splines oscillate)
4. No validation (arbitrage checks essential)
5. Mixing curve types (Treasury vs. swap, different credit)
6. Blindly trusting stressed market data (March 2020)
7. Over-smoothing (lose important curve features)
8. Under-smoothing (noise amplified)

### Validation Checklist

**Before using curve:**

✅ All instruments priced within $0.10 of market
✅ No arbitrage opportunities >$0.05
✅ Curve shape economically sensible
✅ Forward rates smooth (no spikes)
✅ Cross-validate with swap curve
✅ Sanity check (no negative rates unless justified)
✅ Document methodology (reproducible)

### Applications

**What you can do with curve:**

- **Price bonds:** Any coupon, any maturity
- **Value swaps:** Calculate present value of fixed leg
- **Compute duration:** Sensitivity to rate changes
- **Extract forwards:** Market expectations
- **Identify arbitrage:** Mispriced securities
- **Risk management:** Hedge portfolios, compute Greeks
